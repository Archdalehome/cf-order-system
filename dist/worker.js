...
`}var ze=`
  ...
  </div>`;function ke(t,e,a,d){let m=t||"待办清单",p=e!==!1,r=String(a??"").trim();return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
</html>`}import{connect as Ve}from"cloudflare:sockets";var J="default",Ke="__platform__",je=24*60*60*1e3,Je="该功能为专业版功能：请点击顶栏「订阅」升级为专业用户后使用（试用账号仅限本人使用与添加订单）",R=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,z=10*60,Ie=5,ce=60,qe=5,Qe=1e4,W=3,He=2e4,Fe=8e3;function D(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function M(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function K(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var We=["USD","CNY"];function Ge(t){let e=String(t??"").trim().toUpperCase();return We.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function S(t){try{return await t.json()}catch{return{}}}function Ae(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function x(t){return t?O(t.role)?Ke:t.teamId||J:J}function O(t){return t==="superadmin"}function T(t){return t==="team"}function ae(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function ne(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(ae(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function P(t){return ae(t)&&ne(t)==="active"}function Le(t){return t&&(t.subscribeRequest||t.renewRequest)||null}function N(){return n({error:Je},403)}async function U(t,e){if(!e||e===J)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return T(d.role)?d:null}async function Ye(t,e){return e?T(e.role)?P(e):P(await U(t,x(e))):!1}async function w(t,e){let a=Ae(t);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let m=await e.TODO_KV.get(`user:${d}`);if(!m)return null;let p=JSON.parse(m);if(!O(p.role)){let r=await U(e,x(p));if(r&&r.status==="disabled")return null}return p}function Ze(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function _(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function Y(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),m=d.slice(0,Math.min(2,d.length));return m+"*".repeat(Math.max(1,d.length-m.length))+e.slice(a)}function me(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function Xe(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),d=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${d(a.getUTCMonth()+1)}-${d(a.getUTCDate())} ${d(a.getUTCHours())}:${d(a.getUTCMinutes())}`}function fe(t){let e=Array.isArray(t)?t.slice(0,W):[],a=[];for(let d=0;d<W;d++){let m=String(e[d]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function G(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],d=Array.isArray(a)?a:[a],m=[];for(let p of d){let r=String(p||"").trim();!r||!R.test(r)||r.toLowerCase()!==e&&(m.some(o=>o.toLowerCase()===r.toLowerCase())||m.push(r))}return m}function et(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let d of a){let m=String(d||"").trim();if(m&&R.test(m))return m}return""}async function te(t){let e={};try{let b=await t.TODO_KV.get("emailSettings");b&&(e=JSON.parse(b)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),d=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),p=Number(t.SMTP_PORT||e.smtpPort||465)||465,r=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),o=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),i=String(t.MAIL_FROM||e.from||"").trim(),s=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,l=i||(r&&R.test(r)?r:""),c=a?"resend":r&&o?"smtp":s?"cloudflare":"",u=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),f=fe(e.subQrCodes),g=a?t.RESEND_API_KEY?"env":"kv":r&&o?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":s?"binding":"none";return{provider:c,apiKey:a,from:l,fromName:d,smtpHost:m,smtpPort:p,smtpUser:r,smtpPass:o,binding:s,adminNotifyEmail:u,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:f,devMode:e.devMode===!0,source:g}}function oe(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function Ce(t){let e=String(t||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function Pe(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${oe(e)}?=`}var tt="support@cloudnexus.cn";async function j(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||"待办清单",allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?tt:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim()}}function Oe(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function Ne(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:Oe(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:Oe(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:fe(t.subQrCodes).concat("").slice(0,W),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function rt(t,e){let a="cfmail_"+D().slice(0,16),d=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${Pe(t.fromName)} <${t.from}>`:`<${t.from}>`,p=G(e),r=[`From: ${m}`,`To: <${e.to}>`];return p.length&&r.push(`Cc: ${p.map(o=>`<${o}>`).join(", ")}`),r.push(`Subject: ${Pe(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${D()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),r.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",Ce(oe(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",Ce(oe(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function nt(t,e,a){let d=null;return Promise.race([t,new Promise((m,p)=>{d=setTimeout(()=>{try{a&&a()}catch{}p(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function at(t,e,a){let d=t&&t.message?t.message:String(t),m=(d.match(/\b(\d{3})\b/)||[])[1]||"",p="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",r="";return m==="535"||m==="534"?r="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?r="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?p:""):m==="550"||m==="553"?r="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?r=p:/starttls must be set|secureTransport/i.test(d)?r="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?r="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(r="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${r}`}async function ot(t,e,a){let d=Number(t.smtpPort)||465,m=[d];d===465?m.push(587):d===587&&m.push(465);let p=[];for(let r=0;r<m.length;r++){let o=await st(m[r],t,e,a);if(o.ok)return o;p.push(m.length>1?`端口 ${m[r]}：${o.error}`:o.error)}return{ok:!1,error:p.join("；")}}async function st(t,e,a,d){let m=e.smtpHost||"smtp.qq.com",p=!!(d&&d.local),r=Number(e.smtpTimeoutMs)||(p?Fe:He),o=new TextEncoder,i=new TextDecoder,s=null,l=null,c=null,u="",f=`连接 ${m}:${t}`;async function g(){for(;;){let h=u.indexOf(`
