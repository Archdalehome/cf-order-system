...
`;function ae(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function ge(t){let e=String(t||"").trim();if(!e)return"";let a=ae(e);return`<link rel="icon" href="${a}">
...
`}var Re=`
  ...
  </div>`;function ve(t,e,a,i){let m=t||"待办清单",p=e!==!1,r=String(a??"").trim();return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
${r?`    <div class="hint hint-inline">忘记密码请联系 <span class="support-mail">${ae(r)}</span></div>
...
</html>`}import{connect as _e}from"cloudflare:sockets";var J="default",ze="__platform__",Ve=24*60*60*1e3,Ke="该功能为专业版功能：请点击顶栏「订阅」升级为专业用户后使用（试用账号仅限本人使用与添加订单）",R=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,z=10*60,Te=5,de=60,je=5,Je=1e4,H=3,qe=2e4,Qe=8e3;function D(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function M(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),i=>i.toString(16).padStart(2,"0")).join("")}function K(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var He=["USD","CNY"];function Fe(t){let e=String(t??"").trim().toUpperCase();return He.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function S(t){try{return await t.json()}catch{return{}}}function Be(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function x(t){return t?O(t.role)?ze:t.teamId||J:J}function O(t){return t==="superadmin"}function T(t){return t==="team"}function re(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function te(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(re(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function P(t){return re(t)&&te(t)==="active"}function Me(t){return t&&(t.subscribeRequest||t.renewRequest)||null}function N(){return n({error:Ke},403)}async function U(t,e){if(!e||e===J)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let i=JSON.parse(a);return T(i.role)?i:null}async function We(t,e){return e?T(e.role)?P(e):P(await U(t,x(e))):!1}async function w(t,e){let a=Be(t);if(!a)return null;let i=await e.TODO_KV.get(`session:${a}`);if(!i)return null;let m=await e.TODO_KV.get(`user:${i}`);if(!m)return null;let p=JSON.parse(m);if(!O(p.role)){let r=await U(e,x(p));if(r&&r.status==="disabled")return null}return p}function Ge(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function _(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function W(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let i=e.slice(0,a),m=i.slice(0,Math.min(2,i.length));return m+"*".repeat(Math.max(1,i.length-m.length))+e.slice(a)}function ue(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function Ye(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),i=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${i(a.getUTCMonth()+1)}-${i(a.getUTCDate())} ${i(a.getUTCHours())}:${i(a.getUTCMinutes())}`}function pe(t){let e=Array.isArray(t)?t.slice(0,H):[],a=[];for(let i=0;i<H;i++){let m=String(e[i]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function F(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],i=Array.isArray(a)?a:[a],m=[];for(let p of i){let r=String(p||"").trim();!r||!R.test(r)||r.toLowerCase()!==e&&(m.some(s=>s.toLowerCase()===r.toLowerCase())||m.push(r))}return m}function Ze(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let i of a){let m=String(i||"").trim();if(m&&R.test(m))return m}return""}async function X(t){let e={};try{let b=await t.TODO_KV.get("emailSettings");b&&(e=JSON.parse(b)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),i=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),p=Number(t.SMTP_PORT||e.smtpPort||465)||465,r=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),s=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),o=String(t.MAIL_FROM||e.from||"").trim(),d=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,l=o||(r&&R.test(r)?r:""),c=a?"resend":r&&s?"smtp":d?"cloudflare":"",u=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),f=pe(e.subQrCodes),g=a?t.RESEND_API_KEY?"env":"kv":r&&s?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":d?"binding":"none";return{provider:c,apiKey:a,from:l,fromName:i,smtpHost:m,smtpPort:p,smtpUser:r,smtpPass:s,binding:d,adminNotifyEmail:u,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:f,devMode:e.devMode===!0,source:g}}function ne(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let i=0;i<e.length;i++)a+=String.fromCharCode(e[i]);return btoa(a)}function Se(t){let e=String(t||""),a=[];for(let i=0;i<e.length;i+=76)a.push(e.slice(i,i+76));return a.join(`\r
`)}function Ie(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${ne(e)}?=`}var Xe="support@cloudnexus.cn";async function j(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||"待办清单",allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?Xe:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim()}}function Ce(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function Pe(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:Ce(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:Ce(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:pe(t.subQrCodes).concat("").slice(0,H),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function et(t,e){let a="cfmail_"+D().slice(0,16),i=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${Ie(t.fromName)} <${t.from}>`:`<${t.from}>`,p=F(e),r=[`From: ${m}`,`To: <${e.to}>`];return p.length&&r.push(`Cc: ${p.map(s=>`<${s}>`).join(", ")}`),r.push(`Subject: ${Ie(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${D()}@${i}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),r.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",Se(ne(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",Se(ne(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function tt(t,e,a){let i=null;return Promise.race([t,new Promise((m,p)=>{i=setTimeout(()=>{try{a&&a()}catch{}p(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(i))}function rt(t,e,a){let i=t&&t.message?t.message:String(t),m=(i.match(/\b(\d{3})\b/)||[])[1]||"",p="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",r="";return m==="535"||m==="534"?r="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?r="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?p:""):m==="550"||m==="553"?r="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(i)?r=p:/starttls must be set|secureTransport/i.test(i)?r="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(i)?r="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(i)&&(r="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${i}${r}`}async function nt(t,e,a){let i=Number(t.smtpPort)||465,m=[i];i===465?m.push(587):i===587&&m.push(465);let p=[];for(let r=0;r<m.length;r++){let s=await at(m[r],t,e,a);if(s.ok)return s;p.push(m.length>1?`端口 ${m[r]}：${s.error}`:s.error)}return{ok:!1,error:p.join("；")}}async function at(t,e,a,i){let m=e.smtpHost||"smtp.qq.com",p=!!(i&&i.local),r=Number(e.smtpTimeoutMs)||(p?Qe:qe),s=new TextEncoder,o=new TextDecoder,d=null,l=null,c=null,u="",f=`连接 ${m}:${t}`;async function g(){for(;;){let h=u.indexOf(`
`);if(h!==-1){let y=u.slice(0,h);return u=u.slice(h+1),y.replace(/\r$/,"")}let{value:E,done:C}=await l.read();if(C)throw new Error("连接已被服务器关闭");u+=o.decode(E,{stream:!0})}}async function b(){let h=0,E="";for(;;){let C=await g();if(!(C.length<3)&&(h=parseInt(C.slice(0,3),10)||h,E+=(E?" ":"")+C.slice(4).trim(),C.charAt(3)!=="-"))break}return{code:h,text:E}}async function v(h,E){await c.write(s.encode(h+`\r
`));let C=await b();if(!E.includes(C.code))throw new Error(`服务器返回 ${C.code}${C.text?" "+C.text:""}`);return C}async function k(){f=`连接 ${m}:${t}`;try{await d.opened}catch(B){throw new Error(`建立连接失败：${B&&B.message?B.message:B}`)}let h=await b();if(h.code!==220)throw new Error(`服务器返回 ${h.code} ${h.text}`);let C=`EHLO ${e.smtpEhlo||String(e.from||"").split("@")[1]||"localhost"}`;if(await v(C,[250]),t===587){f="STARTTLS",await v("STARTTLS",[220]);try{l.releaseLock()}catch{}try{c.releaseLock()}catch{}d=d.startTls(),u="",l=d.readable.getReader(),c=d.writable.getWriter(),await v(C,[250])}f="身份认证（SMTP 授权码）",await v("AUTH LOGIN",[334]),await v(ne(e.smtpUser),[334]),await v(ne(e.smtpPass),[235]),f="发件地址",await v(`MAIL FROM:<${e.from}>`,[250]);let y=[a.to].concat(F(a));for(let B=0;B<y.length;B++)f=B===0?"收件地址":"抄送地址",await v(`RCPT TO:<${y[B]}>`,[250,251]);f="发送邮件正文",await v("DATA",[354]);let I=et(e,a).replace(/\r\n\./g,`\r
..`);await c.write(s.encode(`${I}\r
.\r
`));let $=await b();if($.code!==250)throw new Error(`服务器返回 ${$.code}${$.text?" "+$.text:""}`);try{await v("QUIT",[221])}catch{}return{ok:!0,id:""}}try{return d=_e({hostname:m,port:t},{secureTransport:t===587?"starttls":"on"}),l=d.readable.getReader(),c=d.writable.getWriter(),await tt(k(),r,()=>{try{d.close()}catch{}})}catch(h){return{ok:!1,error:rt(h,f,p)}}finally{try{d&&d.close()}catch{}}}async function we(t,e,a,i){if(!e.provider)return{ok:!1,error:"邮件服务未配置：请在控制台「邮件设置」中填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）"};if(!e.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请在控制台「邮件设置」中补全后重试"};if(e.provider==="smtp")return nt(e,a,i);if(e.provider==="resend"){let m=new AbortController,p=setTimeout(()=>m.abort(),Je);try{let r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify(Object.assign({from:e.fromName?`${e.fromName} <${e.from}>`:e.from,to:[a.to],subject:a.subject,text:a.text,html:a.html},F(a).length?{cc:F(a)}:{})),signal:m.signal}),s=await r.json().catch(()=>({}));return r.ok?{ok:!0,id:s.id||""}:{ok:!1,error:`邮件服务返回错误（${r.status}）：${s.message||s.error||"请检查 API Key 与发件域名"}`}}catch(r){return{ok:!1,error:"邮件服务请求失败："+(r&&r.message?r.message:String(r))}}finally{clearTimeout(p)}}try{let m=F(a);return await e.binding.send({from:e.fromName?{name:e.fromName,email:e.from}:e.from,to:m.length?[a.to].concat(m):a.to,subject:a.subject,html:a.html,text:a.text}),{ok:!0,id:""}}catch(m){return{ok:!1,error:"邮件发送失败："+(m&&m.message?m.message:String(m))}}}async function st(t,e,a){let i="待办清单";try{let o=await t.TODO_KV.get("settings");if(o){let d=JSON.parse(o);d&&d.siteName&&(i=d.siteName)}}catch{}let m=e.teamName||e.username,p=`【${i}】邮箱确认码 ${a}（10 分钟内有效）`,r=[`${m}，您好：`,"",`您正在注册「${i}」的团队账号（登录名：${e.username}）。`,`邮箱确认码：${a}`,"有效期：10 分钟。","","请在注册 / 登录页面输入上面的确认码完成验证，验证通过后即可正常登录。","若非本人操作，请忽略本邮件：验证通过前该账号无法登录。","",i].join(`
`),s=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${_(m)}，您好：</p><p>您正在注册「${_(i)}」的团队账号（登录名：<b>${_(e.username)}</b>）。</p><p>邮箱确认码：</p><p style="font-size:26px;font-weight:700;letter-spacing:6px;color:#2383e2;margin:8px 0">${a}</p><p>有效期 <b>10 分钟</b>。请在注册 / 登录页面输入该确认码完成验证，验证通过后即可正常登录。</p><p style="color:#9b9a97;font-size:12px">若非本人操作，请忽略本邮件：验证通过前该账号无法登录。</p></div>`;return{subject:p,text:r,html:s}}async function ot(t,e,a,i,m){let p="待办清单";try{let y=await t.TODO_KV.get("settings");if(y){let I=JSON.parse(y);I&&I.siteName&&(p=I.siteName)}}catch{}let s=m==="renew"?"续费申请":"订阅申请",o=e.teamName||e.username,d=a&&a.plan?a.plan:null,l=d?`${d.term}（￥${d.price} / ${d.days} 天）`:"未选择套餐",c=Ye(a&&a.at?a.at:new Date().toISOString()),u=String(i&&i.subExtraText||"").trim(),f=pe(i&&i.subQrCodes).filter(Boolean),g=`【${p}】${s}已收到：${o} · ${l}`,b=[`${o}，您好：`,"",`我们已收到您的「${s}」，信息如下（本邮件已同步抄送管理员）：`,"","【申请人信息】",`团队名称：${o}`,`登录账号：${e.username}`,`联系人：${e.contact||"（未填写）"}`,`联系邮箱：${e.email||"（未填写）"}`,`提交时间：${c}`,"","【订阅套餐】",`申请类型：${s}`,`套餐：${l}`,`留言：${a&&a.note||"（无）"}`,""];u&&b.push("【说明】",u,""),f.length&&(b.push("【收款二维码】"),f.forEach((y,I)=>b.push(`二维码 ${I+1}：${y}`)),b.push("")),b.push("提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。","如需补充信息，可在系统内重新提交申请（会覆盖上一次）。","",p);let v=b.join(`
`),k=(y,I)=>`<tr><td style="padding:4px 12px 4px 0;color:#6b6b68;white-space:nowrap">${_(y)}</td><td style="padding:4px 0;color:#37352f">${_(I)}</td></tr>`,h=u?`<div style="margin:14px 0 6px;font-weight:600">说明</div><div style="background:#f7f7f5;border-radius:8px;padding:12px;white-space:pre-wrap">${_(u)}</div>`:"",E=f.length?'<div style="margin:16px 0 6px;font-weight:600">收款二维码</div>'+f.map((y,I)=>`<div style="display:inline-block;margin:0 12px 12px 0;text-align:center;vertical-align:top"><img src="${_(y)}" alt="收款二维码 ${I+1}" style="width:180px;height:180px;object-fit:contain;border:1px solid #e9e9e7;border-radius:8px;background:#fff"><div style="font-size:12px;color:#6b6b68;margin-top:4px">二维码 ${I+1}</div></div>`).join(""):"",C=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${_(o)}，您好：</p><p>我们已收到您的「<b>${_(s)}</b>」，信息如下（本邮件已同步抄送管理员）：</p><div style="margin:10px 0 6px;font-weight:600">申请人信息</div><table style="border-collapse:collapse;font-size:14px">`+k("团队名称",o)+k("登录账号",e.username)+k("联系人",e.contact||"（未填写）")+k("联系邮箱",e.email||"（未填写）")+k("提交时间",c)+'</table><div style="margin:14px 0 6px;font-weight:600">订阅套餐</div><table style="border-collapse:collapse;font-size:14px">'+k("申请类型",s)+k("套餐",l)+k("留言",a&&a.note||"（无）")+"</table>"+h+E+'<p style="margin-top:16px;color:#6b6b68;font-size:13px">提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。<br>如需补充信息，可在系统内重新提交申请（会覆盖上一次）。</p></div>';return{subject:g,text:v,html:C}}async function he(t,e,a){let i=await X(t),m=i.devMode||!i.provider&&ue(a);if(!i.provider&&!m)return{ok:!1,error:"邮件服务未配置：请联系超级管理员在控制台「邮件设置」中配置发件服务后再试"};if(i.provider&&!i.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请先联系超级管理员补全配置"};if(!e.email)return{ok:!1,error:"该账号没有可用的注册邮箱，请重新注册"};let p=Date.now(),r=`mailrate:${e.username}`,s=null;try{let c=await t.TODO_KV.get(r);s=c?JSON.parse(c):null}catch{s=null}let o=!!(s&&p-Number(s.windowStart||0)<60*60*1e3);if(o){let c=de*1e3-(p-Number(s.lastSentAt||0));if(c>0)return{ok:!1,error:`确认码刚发送过，请 ${Math.ceil(c/1e3)} 秒后再点「重新发送确认码」`};if(Number(s.count||0)>=je)return{ok:!1,error:"发送过于频繁（1 小时内最多 5 次），请稍后再试"}}let d=Ge();if(m)console.log(`[邮箱确认码] ${e.username} <${e.email}> 确认码：${d}（调试模式，未真实发送邮件）`);else{let c=await st(t,e,d),u=await we(t,i,{to:e.email,...c},{local:ue(a)});if(!u.ok)return{ok:!1,error:u.error}}await t.TODO_KV.put(`emailcode:${e.username}`,JSON.stringify({codeHash:await M(d),email:e.email||"",sentAt:p,attempts:0}),{expirationTtl:z});let l=o?{windowStart:s.windowStart,count:Number(s.count||0)+1,lastSentAt:p}:{windowStart:p,count:1,lastSentAt:p};return await t.TODO_KV.put(r,JSON.stringify(l),{expirationTtl:Math.max(60,Math.ceil((60*60*1e3-(p-Number(l.windowStart)))/1e3))}),{ok:!0,sentAt:p,devCode:m?d:""}}async function it(t,e,a){try{let m=await t.TODO_KV.get(`emailcode:${e.username}`);if(m){let p=JSON.parse(m);if(z*1e3-(Date.now()-Number(p.sentAt||0))>0)return{ok:!0,resent:!1,sentAt:p.sentAt,devCode:""}}}catch{}let i=await he(t,e,a);return i.ok?{ok:!0,resent:!0,sentAt:i.sentAt,devCode:i.devCode}:{ok:!1,error:i.error}}async function dt(t,e,a){let i=`emailcode:${e.username}`,m=String(a||"").trim();if(!m)return{ok:!1,error:"请输入邮箱确认码"};let p=await t.TODO_KV.get(i);if(!p)return{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};let r=null;try{r=JSON.parse(p)}catch{r=null}if(!r||!r.codeHash)return await t.TODO_KV.delete(i),{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};if(Date.now()-Number(r.sentAt||0)>z*1e3)return await t.TODO_KV.delete(i),{ok:!1,error:"确认码已过期（有效期 10 分钟），请点击「重新发送确认码」"};if(Number(r.attempts||0)>=Te)return await t.TODO_KV.delete(i),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"};if(await M(m)!==r.codeHash){r.attempts=Number(r.attempts||0)+1;let s=Te-r.attempts;return s<=0?(await t.TODO_KV.delete(i),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"}):(await t.TODO_KV.put(i,JSON.stringify(r),{expirationTtl:z}),{ok:!1,error:`确认码不正确，请检查后重试（还可尝试 ${s} 次）`})}return{ok:!0}}async function be(t,e){let a=D();return await t.TODO_KV.put(`session:${a}`,e.username,{expirationTtl:60*60*24*7}),new Response(JSON.stringify({ok:!0,username:e.username,role:e.role}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":`token=${a}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60*60*24*7}`}})}async function se(t){let e=await t.TODO_KV.get("user:admin");if(!e){let i={username:"admin",password:await M("admin"),role:"superadmin",createdAt:new Date().toISOString()};await t.TODO_KV.put("user:admin",JSON.stringify(i));return}let a=JSON.parse(e);a.role==="admin"&&(a.role="superadmin",await t.TODO_KV.put("user:admin",JSON.stringify(a)))}async function Ae(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),i=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(p){let r=JSON.parse(p);if(r.role==="producer"||r.role==="customer"||O(r.role)||T(r.role)||x(r)!==e)continue;let s={username:r.username,role:r.role,createdAt:r.createdAt,remark:r.remark||"",dept:r.dept||"",canPlaceOrder:le(r)};r.role==="restricted"&&(s.watched=await ee(t,r.username)),s.mentionsUnread=await ft(t,r.username),i.push(s)}}return i}async function A(t,e,a){let i=await t.TODO_KV.get(`user:${e}`);if(!i)return null;let m=JSON.parse(i);return O(m.role)||x(m)!==a?null:m}async function ct(t){let e=await t.TODO_KV.list({prefix:"user:"}),a=[];for(let i of e.keys){let m=await t.TODO_KV.get(i.name);if(!m)continue;let p=JSON.parse(m);T(p.role)&&a.push({username:p.username,teamName:p.teamName||p.username,contact:p.contact||"",email:p.email||"",remark:p.remark||"",plan:re(p)?"pro":"trial",pro:P(p),status:te(p),rawStatus:p.status||"active",emailVerified:p.emailVerified!==!1,emailVerifiedAt:p.emailVerifiedAt||"",trialEndsAt:p.trialEndsAt||"",expiresAt:re(p)&&p.expiresAt||"",createdAt:p.createdAt,renewedAt:p.renewedAt||"",subscribeRequest:Me(p)})}return a.sort((i,m)=>{let p=i.subscribeRequest&&i.subscribeRequest.at?1:0,r=m.subscribeRequest&&m.subscribeRequest.at?1:0;return p!==r?r-p:i.createdAt<m.createdAt?1:-1}),a}async function q(t,e){let a=await t.TODO_KV.get(`todos:${e}`);return a?JSON.parse(a):[]}async function G(t,e,a){await t.TODO_KV.put(`todos:${e}`,JSON.stringify(a))}async function Q(t,e){let a=await t.TODO_KV.get(`customers:${e}`);return a?JSON.parse(a):[]}async function xe(t,e,a){await t.TODO_KV.put(`customers:${e}`,JSON.stringify(a))}async function V(t,e){let a=!e||e===J?"customerList":`customerList:${e}`,i=await t.TODO_KV.get(a);return i?JSON.parse(i):[]}async function ce(t,e,a){let i=!e||e===J?"customerList":`customerList:${e}`;await t.TODO_KV.put(i,JSON.stringify(a))}async function lt(t,e,a){let i=String(a||"").trim();if(!i)return;let m=await Q(t,e.username);m.some(s=>s.name===i)||(m.push({id:D().slice(0,12),name:i,createdAt:new Date().toISOString()}),await xe(t,e.username,m));let p=x(e),r=await V(t,p);r.some(s=>s.name===i)||(r.push({id:D().slice(0,12),name:i,description:"",createdAt:new Date().toISOString()}),await ce(t,p,r))}var Le=["self","purchased"],ut="self";function Ue(t){let e=String(t??"").trim();return Le.includes(e)?e:ut}async function L(t,e){let a=!e||e===J?"producers":`producers:${e}`,i=await t.TODO_KV.get(a);return(i?JSON.parse(i):[]).map(p=>({id:p.id,username:p.username||p.shortName||"",description:p.description||"",nature:Ue(p.nature),createdAt:p.createdAt}))}async function oe(t,e,a){let i=!e||e===J?"producers":`producers:${e}`;await t.TODO_KV.put(i,JSON.stringify(a))}async function ee(t,e){let a=await t.TODO_KV.get(`watch:${e}`);return a?JSON.parse(a):[]}async function Oe(t,e,a){await t.TODO_KV.put(`watch:${e}`,JSON.stringify(a))}var mt=50;function pt(t){let e=new Set,a=/@([A-Za-z0-9_.-]{3,20})/g,i=String(t||""),m;for(;(m=a.exec(i))!==null;)e.add(m[1]);return[...e]}async function Ne(t,e){let a=[],i=await U(t,e);i&&a.push({username:i.username,role:i.role,isTeamAdmin:!0,label:i.teamName||i.username});let m=await Ae(t,e);for(let p of m)a.push({username:p.username,role:p.role,dept:p.dept||"",isTeamAdmin:!1,label:p.username});return a}async function me(t,e){let a=await t.TODO_KV.get(`mentions:${e}`);return a?JSON.parse(a):[]}async function De(t,e,a){await t.TODO_KV.put(`mentions:${e}`,JSON.stringify(a.slice(0,mt)))}async function $e(t,e){let a=await me(t,e);return{unread:a.filter(i=>!i.read).length,total:a.length}}async function ft(t,e){return(await $e(t,e)).unread}async function gt(t,e,a,i){if(!a||!a.length)return;let m=new Date().toISOString();for(let p of a){let r=await me(t,p);r.unshift({id:D().slice(0,12),from:e.username,todoId:i.todoId,todoTitle:i.todoTitle||"",todoOwner:i.owner||"",text:i.text||"",at:m,read:!1}),await De(t,p,r)}}var bt=["生产部","计划部","采购部","品质部","财务部"],yt="生产部";function wt(t,e){if(e!=="restricted")return"";let a=String(t??"").trim();return bt.includes(a)?a:yt}var ht={superadmin:"超级管理员",team:"团队管理员",editor:"业务部",viewer:"业务主管",restricted:"生产部",producer:"生产方",customer:"客户",superviewer:"总经理"};function ie(t){return t&&(t.dept||ht[t.role]||t.role)||""}function ye(t){return t==="viewer"||t==="restricted"||t==="producer"||t==="customer"}function le(t){return t?T(t.role)?!0:typeof t.canPlaceOrder=="boolean"?t.canPlaceOrder:t.role==="editor"||t.role==="member":!1}function Y(t){return T(t)||t==="superviewer"}async function Z(t,e,a=!1,i=null){let m=await t.TODO_KV.list({prefix:"todos:"}),p=m.keys.map(o=>o.name.replace("todos:","")),r=await Promise.all(p.map(async o=>{let d=await t.TODO_KV.get(`user:${o}`);return d?JSON.parse(d):null})),s=[];for(let o=0;o<p.length;o++){let d=r[o];if(!d||x(d)!==e)continue;let l=await t.TODO_KV.get(m.keys[o].name),c=l?JSON.parse(l):[];for(let u of c){let f=u.status||(u.done?"done":"pending");a&&f==="pending"||i&&!i.includes(u.producerId)||s.push({...u,status:f,owner:p[o]})}}return s.sort((o,d)=>o.createdAt<d.createdAt?1:-1),s}async function xt(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),i=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(!p)continue;let r=null;try{r=JSON.parse(p)}catch{continue}!r||O(r.role)||x(r)===e&&i.push({username:r.username||m.name.replace(/^user:/,""),role:r.role})}return i}async function vt(t,e){let a=0,i=await t.TODO_KV.get(`todos:${e}`);if(i)try{let m=JSON.parse(i);a=Array.isArray(m)?m.length:0}catch{a=0}return await t.TODO_KV.delete(`user:${e}`),await t.TODO_KV.delete(`todos:${e}`),await t.TODO_KV.delete(`customers:${e}`),await t.TODO_KV.delete(`watch:${e}`),await t.TODO_KV.delete(`mentions:${e}`),await t.TODO_KV.delete(`emailcode:${e}`),await t.TODO_KV.delete(`mailrate:${e}`),{todos:a}}async function Et(t,e,a){let i=t.method;if(a==="/api/login"&&i==="POST"){await se(e);let{username:r,password:s}=await S(t);if(!r||!s)return n({error:"请输入用户名和密码"},400);let o=await e.TODO_KV.get(`user:${r}`);if(!o)return n({error:"用户名或密码错误"},401);let d=JSON.parse(o);if(await M(s)!==d.password)return n({error:"用户名或密码错误"},401);if(!O(d.role)){let c=await U(e,x(d));if(c&&te(c)==="disabled")return n({error:"该账户已被停用，请联系超级管理员开通"},403)}if(d.role==="team"&&d.emailVerified===!1){let c=await it(e,d,t);return n({ok:!1,needVerify:!0,username:d.username,email:W(d.email||""),codeTtlSec:z,resendAfterSec:de,devCode:c.devCode||void 0,sendError:c.ok?void 0:c.error,message:c.ok?`确认码${c.resent?"已重新发送":"已发送"}至注册邮箱，请在 ${Math.floor(z/60)} 分钟内输入完成确认`:"确认码发送失败，请稍后点「重新发送确认码」重试"})}return be(e,d)}if(a==="/api/register"&&i==="POST"){if(await se(e),!(await j(e)).allowRegister)return n({error:"系统当前已关闭新用户注册，如需开通请联系超级管理员"},403);let{teamName:s,username:o,email:d,password:l,confirmPassword:c,contact:u,remark:f}=await S(t),g=(o||"").trim(),b=(s||"").trim(),v=String(d||"").trim(),k=String(l||"");if(!b)return n({error:"请输入团队名称"},400);if(!g)return n({error:"请输入登录用户名"},400);if(!/^[A-Za-z0-9_.-]{3,20}$/.test(g))return n({error:"用户名为 3~20 位字母、数字、下划线、点或短横线"},400);if(g==="admin")return n({error:"该用户名不可用，请更换"},400);if(!v)return n({error:"请输入邮箱"},400);if(v.length>60)return n({error:"邮箱长度不能超过 60 个字符"},400);if(!R.test(v))return n({error:"邮箱格式不正确，请检查后重试"},400);if(!k)return n({error:"请输入密码"},400);if(k.length<6)return n({error:"密码至少 6 位"},400);if(c!==void 0&&k!==String(c))return n({error:"两次输入的密码不一致"},400);if(await e.TODO_KV.get(`user:${g}`))return n({error:"该用户名已被占用，请更换"},400);let E=new Date,C={username:g,password:await M(k),role:"team",teamId:g,teamName:b,contact:String(u||"").trim(),email:v,remark:String(f||"").trim(),emailVerified:!1,plan:"trial",status:"active",trialEndsAt:"",expiresAt:"",createdAt:E.toISOString()};await e.TODO_KV.put(`user:${g}`,JSON.stringify(C));let y=await he(e,C,t);return y.ok?n({ok:!0,needVerify:!0,username:g,role:"team",teamName:b,email:W(v),codeTtlSec:z,resendAfterSec:de,devCode:y.devCode||void 0,unlimitedTrial:!0,message:`确认码已发送至 ${W(v)}，请在 ${Math.floor(z/60)} 分钟内输入完成邮箱确认`}):(await e.TODO_KV.delete(`user:${g}`),n({error:"注册未完成："+y.error},503))}if(a==="/api/register/verify"&&i==="POST"){await se(e);let{username:r,code:s}=await S(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let d=await e.TODO_KV.get(`user:${o}`);if(!d)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(d);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return be(e,l);let c=await dt(e,l,s);return c.ok?(l.emailVerified=!0,l.emailVerifiedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${o}`,JSON.stringify(l)),await e.TODO_KV.delete(`emailcode:${o}`),be(e,l)):n({error:c.error},400)}if(a==="/api/register/resend"&&i==="POST"){await se(e);let{username:r,password:s}=await S(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let d=await e.TODO_KV.get(`user:${o}`);if(!d)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(d);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return n({error:"该账号已完成邮箱确认，可直接登录"},400);if(!s||await M(String(s))!==l.password)return n({error:"密码不正确，无法重新发送确认码"},401);let c=await he(e,l,t);return c.ok?n({ok:!0,email:W(l.email||""),codeTtlSec:z,resendAfterSec:de,devCode:c.devCode||void 0,message:`确认码已重新发送至 ${W(l.email||"")}`}):n({error:c.error},429)}if(a==="/api/logout"&&i==="POST"){let r=Be(t);return r&&await e.TODO_KV.delete(`session:${r}`),new Response(JSON.stringify({ok:!0}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":"token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"}})}if(a==="/api/me"&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s={username:r.username,role:r.role};if(s.canPlaceOrder=le(r),T(r.role)){s.teamId=r.teamId||r.username,s.teamName=r.teamName||r.username,s.contact=r.contact||"",s.plan=re(r)?"pro":"trial",s.pro=P(r),s.trialEndsAt=r.trialEndsAt||"",s.expiresAt=r.expiresAt||"",s.status=te(r);let d=Me(r);s.subscribeRequest=d,s.renewRequest=d}else{let d=await U(e,x(r));s.teamId=x(r),s.teamName=d?d.teamName||d.username:"",s.teamPro=P(d)}let o=await $e(e,r.username);return s.mentionsUnread=o.unread,s.mentionsTotal=o.total,n(s)}if(a==="/api/change-password"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let{oldPassword:s,newPassword:o}=await S(t);return!s||!o?n({error:"请填写完整"},400):await M(s)!==r.password?n({error:"原密码错误"},400):(r.password=await M(o),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0}))}if(a==="/api/settings"&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=await j(e),o=T(r.role)?r:await U(e,x(r));return n({settings:{siteName:s.siteName,allowRegister:s.allowRegister,supportEmail:s.supportEmail,favicon:s.favicon,teamName:o?o.teamName||o.username:""}})}if(a==="/api/settings"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let{siteName:s,teamName:o,allowRegister:d,supportEmail:l,favicon:c}=await S(t);if(o!==void 0){if(!T(r.role))return n({error:"无权限"},403);let b=String(o).trim();return b?(r.teamName=b,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0,settings:{teamName:b}})):n({error:"请输入团队名称"},400)}if(s===void 0&&d===void 0&&l===void 0&&c===void 0)return n({error:"请填写要保存的内容"},400);if(!O(r.role))return n({error:"无权限"},403);let u=await e.TODO_KV.get("settings"),f=u?JSON.parse(u):{};if(s!==void 0){if(!s||!s.trim())return n({error:"请输入网站名称"},400);f.siteName=s.trim()}if(d!==void 0&&(f.allowRegister=d===!0||d==="true"),l!==void 0){let b=String(l||"").trim();if(b.length>60)return n({error:"联系邮箱长度不能超过 60 个字符"},400);if(b&&!R.test(b))return n({error:"联系邮箱格式不正确，请检查后重试（留空则不显示该提示）"},400);f.supportEmail=b}if(c!==void 0){let b=String(c||"").trim();if(b.length>300)return n({error:"网站图标链接不能超过 300 个字符"},400);let v=K(b);if(v===null)return n({error:"网站图标需填写以 http:// 或 https:// 开头的图片链接（留空则使用默认图标）"},400);f.favicon=v}await e.TODO_KV.put("settings",JSON.stringify(f));let g=await j(e);return n({ok:!0,settings:{siteName:g.siteName,allowRegister:g.allowRegister,supportEmail:g.supportEmail,favicon:g.favicon}})}if((a==="/api/subscribe-request"||a==="/api/renew-request")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let{note:s,contact:o,plan:d}=await S(t);o!==void 0&&(r.contact=String(o||"").trim().slice(0,50));let l=d&&typeof d=="object"?d:null,c=l&&Number(l.days)>0?Math.floor(Number(l.days)):0,u={at:new Date().toISOString(),kind:P(r)?"renew":"subscribe",plan:l&&c?{id:String(l.id||"").slice(0,20),term:String(l.term||"").slice(0,20),price:Number(l.price)>=0?Number(l.price):0,days:c}:null,note:String(s||"").trim().slice(0,200),contact:r.contact||""};r.subscribeRequest=u,delete r.renewRequest,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r));let f={ok:!1,to:r.email||"",cc:[],error:""};try{if(!r.email)f.error="该账号没有可用邮箱，无法发送申请邮件";else{let g=await X(e),b=await j(e),v=Ze(g,b);if(f.cc=F({to:r.email,cc:v?[v]:[]}),g.devMode)f.ok=!0,f.devMode=!0;else{let k=await ot(e,r,u,g,u.kind),h=await we(e,g,{to:r.email,cc:f.cc,subject:k.subject,text:k.text,html:k.html},{local:ue(t)});f.ok=!!h.ok,h.ok||(f.error=h.error||"邮件发送失败")}}}catch(g){f.ok=!1,f.error="邮件发送异常："+(g&&g.message?g.message:String(g))}return console.log(`[订阅申请邮件] ${r.username} -> ${f.to||"-"}`+(f.cc&&f.cc.length?`（抄送 ${f.cc.join(", ")}）`:"")+` ${f.ok?"成功":"失败："+f.error}`),n({ok:!0,subscribeRequest:u,mail:f})}let m="/api/teams";if(a===m&&i==="GET"){let r=await w(t,e);return r?O(r.role)?n({teams:await ct(e)}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a.startsWith(`${m}/`)&&a.endsWith("/status")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/status","")),{status:o}=await S(t);if(!["active","disabled"].includes(o))return n({error:"无效的状态"},400);let d=await U(e,s);return d?(d.status=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(d)),n({ok:!0,status:te(d),expiresAt:d.expiresAt||d.trialEndsAt||""})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/renew")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/renew","")),o=await S(t),d=Number(o.days)>0?Math.floor(Number(o.days)):30;if(d>3650)return n({error:"续费天数过大"},400);let l=await U(e,s);if(!l)return n({error:"团队用户不存在"},404);let c=Date.now(),u=new Date(l.expiresAt||"").getTime(),f=Number.isNaN(u)||u<c?c:u,g=new Date(f+d*Ve).toISOString();return l.expiresAt=g,l.status="active",l.plan="pro",l.renewedAt=new Date().toISOString(),delete l.subscribeRequest,delete l.renewRequest,await e.TODO_KV.put(`user:${s}`,JSON.stringify(l)),n({ok:!0,days:d,expiresAt:g,status:"active",pro:!0})}if(a.startsWith(`${m}/`)&&a.endsWith("/reset-password")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/reset-password","")),{newPassword:o}=await S(t);if(!o||!String(o).trim())return n({error:"请输入新密码"},400);let d=await U(e,s);return d?(d.password=await M(String(o).trim()),await e.TODO_KV.put(`user:${s}`,JSON.stringify(d)),n({ok:!0})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/remark")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/remark","")),{remark:o}=await S(t),d=await U(e,s);return d?(d.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(d)),n({ok:!0,remark:d.remark})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.slice(m.length+1));if(!s||s.indexOf("/")!==-1)return n({error:"接口不存在"},404);let o=await U(e,s);if(!o)return n({error:"团队用户不存在"},404);if(o.status!=="disabled")return n({error:"请先「停用」该团队用户，确认无误后再删除"},400);let d=await xt(e,s),l=d.reduce((g,b)=>(g[b.role]=(g[b.role]||0)+1,g),{}),c=0;for(let g of d){let b=await vt(e,g.username);c+=b.todos}let u=await L(e,s),f=await V(e,s);return await e.TODO_KV.delete(`producers:${s}`),await e.TODO_KV.delete(`customerList:${s}`),n({ok:!0,deleted:{team:s,teamName:o.teamName||s,accounts:d.length,members:d.filter(g=>!["team","producer","customer"].includes(g.role)).length,producers:u.length,customers:f.length,todos:c,roles:l}})}if(a==="/api/admin/email-settings"&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=await X(e);return n({settings:Pe(s,e)})}if(a==="/api/admin/email-settings"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let{from:s,fromName:o,apiKey:d,devMode:l,smtpHost:c,smtpPort:u,smtpUser:f,smtpPass:g,adminNotifyEmail:b,subExtraText:v,subQrCodes:k}=await S(t),h=await e.TODO_KV.get("emailSettings"),E=h?JSON.parse(h):{};if(s!==void 0){let y=String(s||"").trim();if(y&&!R.test(y))return n({error:"发件邮箱格式不正确，请检查后重试"},400);E.from=y}if(o!==void 0&&(E.fromName=String(o||"").trim().slice(0,30)),d!==void 0){let y=String(d||"").trim();y==="-"?delete E.apiKey:y&&(E.apiKey=y)}if(c!==void 0){let y=String(c||"").trim();if(y&&!/^[A-Za-z0-9.-]{1,60}$/.test(y))return n({error:"SMTP 服务器地址格式不正确（例如 smtp.qq.com）"},400);E.smtpHost=y||"smtp.qq.com"}if(u!==void 0){let y=String(u??"").trim(),I=Number(y)||0;if(y&&(I<1||I>65535))return n({error:"SMTP 端口需为 1~65535 的数字（QQ 邮箱用 465，或 587 + STARTTLS）"},400);E.smtpPort=I||465}if(f!==void 0){let y=String(f||"").trim();if(y&&!R.test(y))return n({error:"SMTP 账号需填写完整邮箱地址（例如 xxx@qq.com）"},400);E.smtpUser=y}if(g!==void 0){let y=String(g||"").trim();y==="-"?delete E.smtpPass:y&&(E.smtpPass=y)}if(l!==void 0&&(E.devMode=!!l),b!==void 0){let y=String(b||"").trim();if(y&&!R.test(y))return n({error:"管理员提醒邮箱格式不正确（留空则默认抄送到发件邮箱）"},400);E.adminNotifyEmail=y}if(v!==void 0){let y=String(v||"").replace(/\r\n/g,`
`).trim();if(y.length>1e3)return n({error:"自定义说明不能超过 1000 个字符"},400);E.subExtraText=y}if(k!==void 0){let y=Array.isArray(k)?k.slice(0,H):[];if(y.length&&k.length>H)return n({error:`收款二维码最多 ${H} 个`},400);for(let $=0;$<y.length;$++){let B=String(y[$]||"").trim();if(B){if(B.length>300)return n({error:`收款二维码链接太长（第 ${$+1} 个，最多 300 个字符）`},400);if(K(B)===null)return n({error:`收款二维码需填写以 http:// 或 https:// 开头的图片链接（第 ${$+1} 个，留空则不显示）`},400)}}let I=pe(y);for(;I.length&&!I[I.length-1];)I.pop();E.subQrCodes=I}await e.TODO_KV.put("emailSettings",JSON.stringify(E));let C=await X(e);return n({ok:!0,settings:Pe(C,e)})}if(a==="/api/admin/email-test"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let{to:s}=await S(t),o=String(s||"").trim();if(!o)return n({error:"请输入测试收件邮箱"},400);if(!R.test(o))return n({error:"测试收件邮箱格式不正确"},400);let d=await X(e);if(d.devMode)return n({ok:!0,devMode:!0,message:"当前为调试模式（不真实发送邮件），无需测试发信"});if(!d.provider)return n({error:"邮件服务未配置：请填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）并保存后，再发送测试邮件"},400);let l=d.provider==="smtp"?"QQ 邮箱 SMTP":d.provider==="resend"?"Resend":"Cloudflare 邮件绑定",c=await we(e,d,{to:o,subject:"【测试】邮件服务配置正常",text:"这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。",html:"<p>这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。</p>"},{local:ue(t)});return c.ok?n({ok:!0,message:`测试邮件已通过 ${l} 发送至 ${o}，请查收（若没收到，请检查垃圾邮件箱）`}):n({error:c.error},400)}if(a==="/api/users"&&i==="GET"){let r=await w(t,e);return r?T(r.role)?P(r)?n({users:await Ae(e,x(r))}):N():n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/users"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let{username:s,password:o,role:d,dept:l}=await S(t);if(!s||!o)return n({error:"请填写用户名和密码"},400);let c=["restricted","superviewer"].includes(d)?d:"editor";if(await e.TODO_KV.get(`user:${s}`))return n({error:"用户名已存在"},400);let f={username:s,password:await M(o),role:c,teamId:x(r),createdAt:new Date().toISOString()},g=wt(l,c);return g&&(f.dept=g),await e.TODO_KV.put(`user:${s}`,JSON.stringify(f)),n({ok:!0})}if(a.startsWith("/api/users/")&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace("/api/users/",""));return await A(e,s,x(r))?(await e.TODO_KV.delete(`user:${s}`),await e.TODO_KV.delete(`todos:${s}`),await e.TODO_KV.delete(`customers:${s}`),await e.TODO_KV.delete(`watch:${s}`),await e.TODO_KV.delete(`mentions:${s}`),n({ok:!0})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/reset-password")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/users/","").replace("/reset-password","")),{newPassword:d}=await S(t);if(!d||!d.trim())return n({error:"请输入新密码"},400);let l=await e.TODO_KV.get(`user:${o}`);if(!l){let f=(await L(e,s)).find(v=>v.username===o);if(f)return await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await M(d),role:"producer",producerId:f.id,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0});let b=(await V(e,s)).find(v=>v.name===o);return b?(await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await M(d),role:"customer",customerId:b.id,customerName:b.name,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0})):n({error:"成员不存在"},404)}let c=JSON.parse(l);return O(c.role)||x(c)!==s?n({error:"成员不存在"},404):(c.password=await M(d),await e.TODO_KV.put(`user:${o}`,JSON.stringify(c)),n({ok:!0}))}if(a.startsWith("/api/users/")&&a.endsWith("/remark")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace("/api/users/","").replace("/remark","")),{remark:o}=await S(t),d=await A(e,s,x(r));return d?(d.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(d)),n({ok:!0,remark:d.remark})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/order-permission")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace("/api/users/","").replace("/order-permission","")),{canPlaceOrder:o}=await S(t);if(typeof o!="boolean")return n({error:"请传入 true（有）或 false（无）"},400);let d=await A(e,s,x(r));return d?(d.canPlaceOrder=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(d)),n({ok:!0,canPlaceOrder:o})):n({error:"成员不存在"},404)}if(a.startsWith("/api/customers/")&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/customers/",""));return s!==r.username&&!(T(r.role)?await A(e,s,x(r)):null)?n({error:"无权限"},403):n({customers:await Q(e,s)})}if(a.startsWith("/api/customers/")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace("/api/customers/",""));if(!await A(e,s,x(r)))return n({error:"成员不存在"},404);let{name:d,globalId:l}=await S(t);if(!d||!d.trim())return n({error:"请输入客户名称"},400);let c=await Q(e,s),u=d.trim(),f=l?String(l).trim():"";if(c.some(b=>b.name===u||f&&b.id===f))return n({error:"客户已存在"},400);let g={id:f||D().slice(0,12),name:u,createdAt:new Date().toISOString()};return c.push(g),await xe(e,s,c),n({ok:!0,customer:g})}if(a.startsWith("/api/customers/")&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace("/api/customers/","")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let d=s.slice(0,o),l=s.slice(o+1);if(!await A(e,d,x(r)))return n({error:"成员不存在"},404);let u=await Q(e,d);return u=u.filter(f=>f.id!==l),await xe(e,d,u),n({ok:!0})}if(a==="/api/customer-list"&&i==="GET"){let r=await w(t,e);return r?!T(r.role)||!P(r)?N():n({customers:await V(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/customer-list"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=x(r),{name:o,password:d,description:l}=await S(t);if(!o||!o.trim())return n({error:"请输入用户名"},400);if(!d||!String(d).trim())return n({error:"请输入密码"},400);let c=await V(e,s),u=o.trim();if(c.some(b=>b.name===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被占用"},400);let g={id:D().slice(0,12),name:u,description:(l||"").trim(),createdAt:new Date().toISOString()};return c.push(g),await ce(e,s,c),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await M(String(d).trim()),role:"customer",customerId:g.id,customerName:u,teamId:s,createdAt:g.createdAt})),n({ok:!0,customer:g})}if(a.startsWith("/api/customer-list/")&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/customer-list/","")),d=await V(e,s),l=d.find(c=>c.id===o);return d=d.filter(c=>c.id!==o),await ce(e,s,d),l&&l.name&&await e.TODO_KV.delete(`user:${l.name}`),n({ok:!0})}if(a.startsWith("/api/customer-list/")&&a.endsWith("/description")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/customer-list/","").replace("/description","")),{description:d}=await S(t),l=await V(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(d||"").trim(),await ce(e,s,l),n({ok:!0,description:c.description})):n({error:"客户不存在"},404)}if(a==="/api/producers"&&i==="GET"){let r=await w(t,e);return r?n({producers:await L(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/producers"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),{username:o,password:d,description:l,nature:c}=await S(t),u=(o||"").trim();if(!u)return n({error:"请输入用户名"},400);if(!d||!String(d).trim())return n({error:"请输入密码"},400);let f=await L(e,s);if(f.some(v=>v.username===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被成员占用"},400);let b={id:D().slice(0,12),username:u,description:(l||"").trim(),nature:Ue(c),createdAt:new Date().toISOString()};return f.push(b),await oe(e,s,f),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await M(String(d).trim()),role:"producer",producerId:b.id,teamId:s,createdAt:b.createdAt})),n({ok:!0,producer:b})}if(a.startsWith("/api/producers/")&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","")),d=await L(e,s),l=d.find(c=>c.id===o);return d=d.filter(c=>c.id!==o),await oe(e,s,d),l&&l.username&&await e.TODO_KV.delete(`user:${l.username}`),n({ok:!0})}if(a.startsWith("/api/producers/")&&a.endsWith("/description")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/description","")),{description:d}=await S(t),l=await L(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(d||"").trim(),await oe(e,s,l),n({ok:!0,description:c.description})):n({error:"生产方不存在"},404)}if(a.startsWith("/api/producers/")&&a.endsWith("/nature")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/nature","")),{nature:d}=await S(t),l=String(d??"").trim();if(!Le.includes(l))return n({error:"生产方性质只能是「自产」或「外购」"},400);let c=await L(e,s),u=c.find(f=>f.id===o);return u?(u.nature=l,await oe(e,s,c),n({ok:!0,nature:u.nature})):n({error:"生产方不存在"},404)}let p="/api/watch/";if(a.startsWith(p)&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=x(r),o=decodeURIComponent(a.replace(p,""));if(o!==r.username&&!(T(r.role)?await A(e,o,s):null))return n({error:"无权限"},403);let d=await ee(e,o),l=await L(e,s),c=d.map(u=>{let f=l.find(g=>g.id===u);return f||{id:u,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:c})}if(a.startsWith(p)&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=x(r),o=decodeURIComponent(a.replace(p,"")),d=await A(e,o,s);if(!d)return n({error:"成员不存在"},404);let{producerId:l}=await S(t);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await L(e,s)).some(g=>g.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);if(d.role!=="restricted")return n({error:"该成员不是「生产部」成员，无法授权可观察的生产方"},400);let f=await ee(e,o);return f.includes(c)?n({error:"已授权该生产方"},400):(f.push(c),await Oe(e,o,f),n({ok:!0,producerIds:f}))}if(a.startsWith(p)&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!P(r))return N();let s=decodeURIComponent(a.replace(p,"")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let d=s.slice(0,o),l=s.slice(o+1);if(!await A(e,d,x(r)))return n({error:"成员不存在"},404);let u=await ee(e,d);return u=u.filter(f=>f!==l),await Oe(e,d,u),n({ok:!0,producerIds:u})}if(a==="/api/team-members"&&i==="GET"){let r=await w(t,e);return r?n({members:await Ne(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/mentions"&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=await me(e,r.username);return n({items:s,unread:s.filter(o=>!o.read).length,total:s.length})}if(a==="/api/mentions/read"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=await S(t),o=s&&s.id?String(s.id).trim():"",d=!(s&&s.read===!1),l=await me(e,r.username),c=0;for(let u of l)u.read!==d&&(o&&u.id!==o||(u.read=d,c++));return c&&await De(e,r.username,l),n({ok:!0,changed:c,read:d,unread:l.filter(u=>!u.read).length,total:l.length})}if(a==="/api/todos"&&i==="GET"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=x(r);if(Y(r.role))return n({todos:await Z(e,s,!1),readonly:!1,allUsers:!0});if(r.role==="viewer")return n({todos:await Z(e,s,!0),readonly:!0,allUsers:!0});if(r.role==="restricted"){let o=await ee(e,r.username);return n({todos:await Z(e,s,!0,o),readonly:!0,allUsers:!0})}if(r.role==="producer"){let d=(await L(e,s)).find(l=>l.id===r.producerId);return n({todos:await Z(e,s,!0,d?[d.id]:[]),readonly:!0,allUsers:!0})}if(r.role==="customer"){let o=r.customerName||"",d=o?await Z(e,s,!0):[];return n({todos:d.filter(l=>l.customer===o),readonly:!0,allUsers:!0})}return n({todos:await q(e,r.username),readonly:!1})}if(a==="/api/todos"&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!le(r))return n({error:ie(r)+"无「生产单下单权限」，请联系团队管理员在「成员管理」中开通"},403);let{title:s,customer:o,dueDate:d,amount:l,orderUrl:c,purchaseUrl:u,currency:f}=await S(t);if(!o||!o.trim())return n({error:"选择客户"},400);if(!s||!s.trim())return n({error:"请输入主题"},400);if(!d||!String(d).trim())return n({error:"选择交期"},400);let g=K(c);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);let b=K(u);if(b===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);if(l==null||String(l).trim()==="")return n({error:"输入金额"},400);let v=Number(l);if(Number.isNaN(v)||v<0)return n({error:"金额必须为非负数字"},400);let k=o.trim();if(T(r.role)&&!P(r))await lt(e,r,k);else if(T(r.role)){let C=await V(e,x(r)),y=await Q(e,r.username);if(!C.some(I=>I.name===k)&&!y.some(I=>I.name===k))return n({error:"客户不存在，请先在「客户管理」中添加"},400)}else if(!(await Q(e,r.username)).some(y=>y.name===k))return n({error:"客户不存在，请联系管理员添加"},400);let h=await q(e,r.username),E={id:D().slice(0,12),customer:o.trim(),title:s.trim(),dueDate:String(d).trim(),amount:v,currency:Fe(f),orderUrl:g,purchaseUrl:b,notes:[],status:"pending",done:!1,createdAt:new Date().toISOString()};return h.unshift(E),await G(e,r.username,h),n({ok:!0,todo:E})}if(a.startsWith("/api/todos/")&&a.endsWith("/notes")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/notes","")),o=await S(t),d=o.text;if(!d||!d.trim())return n({error:"请输入备注内容"},400);let l=r.username;if(ye(r.role)||Y(r.role)){let h=o.owner||r.username;if(h!==r.username&&!await A(e,h,x(r)))return n({error:"无权操作该用户的待办"},403);l=h}let c=await q(e,l),u=c.findIndex(h=>h.id===s);if(u===-1)return n({error:"未找到"},404);Array.isArray(c[u].notes)||(c[u].notes=[]);let f=await Ne(e,x(r)),g=new Set(f.map(h=>h.username)),b=new Set;if(Array.isArray(o.mentions))for(let h of o.mentions){let E=String(h||"").trim();E&&g.has(E)&&b.add(E)}for(let h of pt(d))g.has(h)&&b.add(h);b.delete(r.username);let v=[...b],k={id:D().slice(0,12),text:d.trim(),author:r.username,createdAt:new Date().toISOString(),mentions:v};return c[u].notes.push(k),await G(e,l,c),await gt(e,r,v,{todoId:c[u].id,todoTitle:c[u].title,owner:l,text:k.text}),n({ok:!0,note:k})}if(a.startsWith("/api/todos/")&&i==="PUT"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(ye(r.role))return n({error:ie(r)+"无编辑权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=await S(t);if((typeof o.status=="string"||typeof o.done=="boolean")&&!Y(r.role))return n({error:"只有团队管理员或总经理可以改变待办状态"},403);let l=r.username;if(Y(r.role)&&o.owner&&o.owner!==r.username){if(!await A(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);l=o.owner}let c=await q(e,l),u=c.findIndex(f=>f.id===s);if(u===-1)return n({error:"未找到"},404);if(typeof o.title=="string"){let f=o.title.trim();if(!f)return n({error:"请输入 PO# / 主题"},400);c[u].title=f}if(typeof o.dueDate=="string"||o.amount!==void 0||o.orderUrl!==void 0||o.purchaseUrl!==void 0){if((c[u].status||(c[u].done?"done":"pending"))!=="pending")return n({error:"仅「待确认」的待办可修改交期/金额/文件链接"},403);if(typeof o.dueDate=="string"){let g=o.dueDate.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(g))return n({error:"请输入正确的交期"},400);c[u].dueDate=g}if(o.amount!==void 0&&o.amount!==null&&String(o.amount).trim()!==""){let g=Number(o.amount);if(Number.isNaN(g)||g<0)return n({error:"金额必须为非负数字"},400);c[u].amount=g}if(o.orderUrl!==void 0){let g=K(o.orderUrl);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);c[u].orderUrl=g}if(o.purchaseUrl!==void 0){let g=K(o.purchaseUrl);if(g===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);c[u].purchaseUrl=g}}if(typeof o.producerId=="string"&&o.producerId.trim()){let g=(await L(e,x(r))).find(b=>b.id===o.producerId.trim());if(!g)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);c[u].producerId=g.id,c[u].producerName=g.username,c[u].producerAssignedAt=new Date().toISOString()}if(typeof o.status=="string"){if(!["pending","doing","done"].includes(o.status))return n({error:"无效的状态"},400);c[u].status=o.status,c[u].done=o.status==="done"}else typeof o.done=="boolean"&&(c[u].done=o.done,c[u].status=o.done?"done":"doing");return c[u].status==="doing"&&!c[u].producerId&&await We(e,r)?n({error:"请为该待办指定生产方（生产方来自「生产方管理」）"},400):(await G(e,l,c),n({ok:!0,todo:c[u]}))}if(a.startsWith("/api/todos/")&&a.endsWith("/purchase-link")&&i==="POST"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(!le(r))return n({error:ie(r)+"无「生产单下单权限」，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/purchase-link","")),o=await S(t),d=r.username;if(o.owner&&o.owner!==r.username){if(!await A(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);d=o.owner}let l=await q(e,d),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].purchaseUrl)return n({error:"该订单已有采购文件链接，如需修改请联系团队管理员"},403);let u=K(o.purchaseUrl);return u===null?n({error:"采购文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].purchaseUrl=u,await G(e,d,l),n({ok:!0,purchaseUrl:u})):n({error:"请输入采购文件链接"},400)}if(a.startsWith("/api/todos/")&&i==="DELETE"){let r=await w(t,e);if(!r)return n({error:"未登录"},401);if(ye(r.role))return n({error:ie(r)+"无删除权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=r.username;if(Y(r.role)){let u=new URL(t.url).searchParams.get("owner");if(u&&u!==r.username){if(!await A(e,u,x(r)))return n({error:"无权操作该用户的待办"},403);o=u}}let d=await q(e,o),l=d.find(u=>u.id===s);return l?(l.status||(l.done?"done":"pending"))!=="pending"?n({error:"该事件已进入「进行中/已完成」状态，无法删除"},403):(d=d.filter(u=>u.id!==s),await G(e,o,d),n({ok:!0})):n({error:"未找到"},404)}return n({error:"接口不存在"},404)}var It={async fetch(t,e,a){let i=new URL(t.url),m=i.pathname;if(m.startsWith("/api/"))try{return await Et(t,e,m)}catch(p){return n({error:"服务器错误: "+p.message},500)}if(m==="/"||m==="/login"){let p=await j(e);return new Response(ve(p.siteName,p.allowRegister,p.supportEmail,p.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/todos"){let p=await j(e);return new Response(Ee(p.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/admin"){let p=await w(t,e);if(!p)return Response.redirect(i.origin+"/",302);if(!O(p.role))return Response.redirect(i.origin+"/todos",302);let r=await j(e);return new Response(ke(r.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}return new Response("Not Found",{status:404})}};export{It as default};
