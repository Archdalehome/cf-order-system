...
`;function X(r){return String(r??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function ce(r){let e=String(r||"").trim();if(!e)return"";let a=X(e);return`<link rel="icon" href="${a}">
...
`}var Le=`
  ...
  </div>`;function be(r,e,a,d){let p=r||"待办清单",m=e!==!1,t=String(a??"").trim();return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
${t?`    <div class="hint hint-inline">忘记密码请联系 <span class="support-mail">${X(t)}</span></div>
...
</html>`}function we(r){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}import{connect as Ue}from"cloudflare:sockets";var _="default",De="__platform__",Re=24*60*60*1e3,$e="该功能为专业版功能：请点击顶栏「订阅」升级为专业用户后使用（试用账号仅限本人使用与添加订单）",K=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,U=10*60,he=5,ne=60,_e=5,ze=1e4,Ve=2e4,Ke=8e3;function L(){let r=new Uint8Array(24);return crypto.getRandomValues(r),Array.from(r,e=>e.toString(16).padStart(2,"0")).join("")}async function O(r){let e=new TextEncoder().encode(r+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function z(r){let e=String(r??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var je=["USD","CNY"];function qe(r){let e=String(r??"").trim().toUpperCase();return je.includes(e)?e:"USD"}function n(r,e=200){return new Response(JSON.stringify(r),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function k(r){try{return await r.json()}catch{return{}}}function Ie(r){let a=(r.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function x(r){return r?I(r.role)?De:r.teamId||_:_}function I(r){return r==="superadmin"}function v(r){return r==="team"}function Y(r){return!!r&&(r.plan==="pro"||r.plan==="paid")}function G(r){if(!r)return"active";if(r.status==="disabled")return"disabled";if(Y(r)){let e=r.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function E(r){return Y(r)&&G(r)==="active"}function Ce(r){return r&&(r.subscribeRequest||r.renewRequest)||null}function P(){return n({error:$e},403)}async function A(r,e){if(!e||e===_)return null;let a=await r.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return v(d.role)?d:null}async function Je(r,e){return e?v(e.role)?E(e):E(await A(r,x(e))):!1}async function h(r,e){let a=Ie(r);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let p=await e.TODO_KV.get(`user:${d}`);if(!p)return null;let m=JSON.parse(p);if(!I(m.role)){let t=await A(e,x(m));if(t&&t.status==="disabled")return null}return m}function He(){let r=new Uint32Array(1);return crypto.getRandomValues(r),String(r[0]%1e6).padStart(6,"0")}function le(r){return String(r??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function J(r){let e=String(r||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),p=d.slice(0,Math.min(2,d.length));return p+"*".repeat(Math.max(1,d.length-p.length))+e.slice(a)}function pe(r){try{let e=new URL(r.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}async function ae(r){let e={};try{let f=await r.TODO_KV.get("emailSettings");f&&(e=JSON.parse(f)||{})}catch{e={}}let a=String(r.RESEND_API_KEY||e.apiKey||"").trim(),d=String(r.MAIL_FROM_NAME||e.fromName||"").trim(),p=String(r.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),m=Number(r.SMTP_PORT||e.smtpPort||465)||465,t=String(r.SMTP_USER||r.QQ_MAIL_USER||e.smtpUser||"").trim(),s=String(r.SMTP_PASS||r.QQ_MAIL_PASS||e.smtpPass||"").trim(),o=String(r.MAIL_FROM||e.from||"").trim(),i=r.SEND_EMAIL&&typeof r.SEND_EMAIL.send=="function"?r.SEND_EMAIL:null,l=o||(t&&K.test(t)?t:""),c=a?"resend":t&&s?"smtp":i?"cloudflare":"",u=a?r.RESEND_API_KEY?"env":"kv":t&&s?r.SMTP_USER||r.SMTP_PASS||r.QQ_MAIL_USER||r.QQ_MAIL_PASS?"env":"kv":i?"binding":"none";return{provider:c,apiKey:a,from:l,fromName:d,smtpHost:p,smtpPort:m,smtpUser:t,smtpPass:s,binding:i,devMode:e.devMode===!0,source:u}}function Z(r){let e=new TextEncoder().encode(String(r??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function xe(r){let e=String(r||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function ve(r){let e=String(r??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${Z(e)}?=`}var Fe="support@cloudnexus.cn";async function q(r){let e={};try{let a=await r.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||"待办清单",allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?Fe:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim()}}function ke(r){let e=String(r||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function Ee(r,e){return{provider:r.provider,source:r.source,from:r.from,fromName:r.fromName,hasApiKey:!!r.apiKey,apiKeyMasked:ke(r.apiKey),smtpHost:r.smtpHost,smtpPort:r.smtpPort,smtpUser:r.smtpUser,hasSmtpPass:!!r.smtpPass,smtpPassMasked:ke(r.smtpPass),devMode:r.devMode,envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function Qe(r,e){let a="cfmail_"+L().slice(0,16),d=String(r.from||"localhost").split("@")[1]||"localhost";return[`From: ${r.fromName?`${ve(r.fromName)} <${r.from}>`:`<${r.from}>`}`,`To: <${e.to}>`,`Subject: ${ve(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${L()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,"",`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",xe(Z(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",xe(Z(e.html||e.text||"")),`--${a}--`,""].join(`\r
`)}function We(r,e,a){let d=null;return Promise.race([r,new Promise((p,m)=>{d=setTimeout(()=>{try{a&&a()}catch{}m(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function Ge(r,e,a){let d=r&&r.message?r.message:String(r),p=(d.match(/\b(\d{3})\b/)||[])[1]||"",m="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",t="";return p==="535"||p==="534"?t="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":p==="530"?t="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?m:""):p==="550"||p==="553"?t="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?t=m:/starttls must be set|secureTransport/i.test(d)?t="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?t="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(t="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${t}`}async function Ye(r,e,a){let d=Number(r.smtpPort)||465,p=[d];d===465?p.push(587):d===587&&p.push(465);let m=[];for(let t=0;t<p.length;t++){let s=await Ze(p[t],r,e,a);if(s.ok)return s;m.push(p.length>1?`端口 ${p[t]}：${s.error}`:s.error)}return{ok:!1,error:m.join("；")}}async function Ze(r,e,a,d){let p=e.smtpHost||"smtp.qq.com",m=!!(d&&d.local),t=Number(e.smtpTimeoutMs)||(m?Ke:Ve),s=new TextEncoder,o=new TextDecoder,i=null,l=null,c=null,u="",f=`连接 ${p}:${r}`;async function g(){for(;;){let b=u.indexOf(`
`);if(b!==-1){let B=u.slice(0,b);return u=u.slice(b+1),B.replace(/\r$/,"")}let{value:T,done:S}=await l.read();if(S)throw new Error("连接已被服务器关闭");u+=o.decode(T,{stream:!0})}}async function y(){let b=0,T="";for(;;){let S=await g();if(!(S.length<3)&&(b=parseInt(S.slice(0,3),10)||b,T+=(T?" ":"")+S.slice(4).trim(),S.charAt(3)!=="-"))break}return{code:b,text:T}}async function w(b,T){await c.write(s.encode(b+`\r
`));let S=await y();if(!T.includes(S.code))throw new Error(`服务器返回 ${S.code}${S.text?" "+S.text:""}`);return S}async function C(){f=`连接 ${p}:${r}`;try{await i.opened}catch($){throw new Error(`建立连接失败：${$&&$.message?$.message:$}`)}let b=await y();if(b.code!==220)throw new Error(`服务器返回 ${b.code} ${b.text}`);let S=`EHLO ${e.smtpEhlo||String(e.from||"").split("@")[1]||"localhost"}`;if(await w(S,[250]),r===587){f="STARTTLS",await w("STARTTLS",[220]);try{l.releaseLock()}catch{}try{c.releaseLock()}catch{}i=i.startTls(),u="",l=i.readable.getReader(),c=i.writable.getWriter(),await w(S,[250])}f="身份认证（SMTP 授权码）",await w("AUTH LOGIN",[334]),await w(Z(e.smtpUser),[334]),await w(Z(e.smtpPass),[235]),f="发件地址",await w(`MAIL FROM:<${e.from}>`,[250]),f="收件地址",await w(`RCPT TO:<${a.to}>`,[250,251]),f="发送邮件正文",await w("DATA",[354]);let B=Qe(e,a).replace(/\r\n\./g,`\r
..`);await c.write(s.encode(`${B}\r
.\r
`));let D=await y();if(D.code!==250)throw new Error(`服务器返回 ${D.code}${D.text?" "+D.text:""}`);try{await w("QUIT",[221])}catch{}return{ok:!0,id:""}}try{return i=Ue({hostname:p,port:r},{secureTransport:r===587?"starttls":"on"}),l=i.readable.getReader(),c=i.writable.getWriter(),await We(C(),t,()=>{try{i.close()}catch{}})}catch(b){return{ok:!1,error:Ge(b,f,m)}}finally{try{i&&i.close()}catch{}}}async function Pe(r,e,a,d){if(!e.provider)return{ok:!1,error:"邮件服务未配置：请在控制台「邮件设置」中填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）"};if(!e.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请在控制台「邮件设置」中补全后重试"};if(e.provider==="smtp")return Ye(e,a,d);if(e.provider==="resend"){let p=new AbortController,m=setTimeout(()=>p.abort(),ze);try{let t=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from:e.fromName?`${e.fromName} <${e.from}>`:e.from,to:[a.to],subject:a.subject,text:a.text,html:a.html}),signal:p.signal}),s=await t.json().catch(()=>({}));return t.ok?{ok:!0,id:s.id||""}:{ok:!1,error:`邮件服务返回错误（${t.status}）：${s.message||s.error||"请检查 API Key 与发件域名"}`}}catch(t){return{ok:!1,error:"邮件服务请求失败："+(t&&t.message?t.message:String(t))}}finally{clearTimeout(m)}}try{return await e.binding.send({from:e.fromName?{name:e.fromName,email:e.from}:e.from,to:a.to,subject:a.subject,html:a.html,text:a.text}),{ok:!0,id:""}}catch(p){return{ok:!1,error:"邮件发送失败："+(p&&p.message?p.message:String(p))}}}async function Xe(r,e,a){let d="待办清单";try{let o=await r.TODO_KV.get("settings");if(o){let i=JSON.parse(o);i&&i.siteName&&(d=i.siteName)}}catch{}let p=e.teamName||e.username,m=`【${d}】邮箱确认码 ${a}（10 分钟内有效）`,t=[`${p}，您好：`,"",`您正在注册「${d}」的团队账号（登录名：${e.username}）。`,`邮箱确认码：${a}`,"有效期：10 分钟。","","请在注册 / 登录页面输入上面的确认码完成验证，验证通过后即可正常登录。","若非本人操作，请忽略本邮件：验证通过前该账号无法登录。","",d].join(`
`),s=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${le(p)}，您好：</p><p>您正在注册「${le(d)}」的团队账号（登录名：<b>${le(e.username)}</b>）。</p><p>邮箱确认码：</p><p style="font-size:26px;font-weight:700;letter-spacing:6px;color:#2383e2;margin:8px 0">${a}</p><p>有效期 <b>10 分钟</b>。请在注册 / 登录页面输入该确认码完成验证，验证通过后即可正常登录。</p><p style="color:#9b9a97;font-size:12px">若非本人操作，请忽略本邮件：验证通过前该账号无法登录。</p></div>`;return{subject:m,text:t,html:s}}async function fe(r,e,a){let d=await ae(r),p=d.devMode||!d.provider&&pe(a);if(!d.provider&&!p)return{ok:!1,error:"邮件服务未配置：请联系超级管理员在控制台「邮件设置」中配置发件服务后再试"};if(d.provider&&!d.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请先联系超级管理员补全配置"};if(!e.email)return{ok:!1,error:"该账号没有可用的注册邮箱，请重新注册"};let m=Date.now(),t=`mailrate:${e.username}`,s=null;try{let c=await r.TODO_KV.get(t);s=c?JSON.parse(c):null}catch{s=null}let o=!!(s&&m-Number(s.windowStart||0)<60*60*1e3);if(o){let c=ne*1e3-(m-Number(s.lastSentAt||0));if(c>0)return{ok:!1,error:`确认码刚发送过，请 ${Math.ceil(c/1e3)} 秒后再点「重新发送确认码」`};if(Number(s.count||0)>=_e)return{ok:!1,error:"发送过于频繁（1 小时内最多 5 次），请稍后再试"}}let i=He();if(p)console.log(`[邮箱确认码] ${e.username} <${e.email}> 确认码：${i}（调试模式，未真实发送邮件）`);else{let c=await Xe(r,e,i),u=await Pe(r,d,{to:e.email,...c},{local:pe(a)});if(!u.ok)return{ok:!1,error:u.error}}await r.TODO_KV.put(`emailcode:${e.username}`,JSON.stringify({codeHash:await O(i),email:e.email||"",sentAt:m,attempts:0}),{expirationTtl:U});let l=o?{windowStart:s.windowStart,count:Number(s.count||0)+1,lastSentAt:m}:{windowStart:m,count:1,lastSentAt:m};return await r.TODO_KV.put(t,JSON.stringify(l),{expirationTtl:Math.max(60,Math.ceil((60*60*1e3-(m-Number(l.windowStart)))/1e3))}),{ok:!0,sentAt:m,devCode:p?i:""}}async function et(r,e,a){try{let p=await r.TODO_KV.get(`emailcode:${e.username}`);if(p){let m=JSON.parse(p);if(U*1e3-(Date.now()-Number(m.sentAt||0))>0)return{ok:!0,resent:!1,sentAt:m.sentAt,devCode:""}}}catch{}let d=await fe(r,e,a);return d.ok?{ok:!0,resent:!0,sentAt:d.sentAt,devCode:d.devCode}:{ok:!1,error:d.error}}async function tt(r,e,a){let d=`emailcode:${e.username}`,p=String(a||"").trim();if(!p)return{ok:!1,error:"请输入邮箱确认码"};let m=await r.TODO_KV.get(d);if(!m)return{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};let t=null;try{t=JSON.parse(m)}catch{t=null}if(!t||!t.codeHash)return await r.TODO_KV.delete(d),{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};if(Date.now()-Number(t.sentAt||0)>U*1e3)return await r.TODO_KV.delete(d),{ok:!1,error:"确认码已过期（有效期 10 分钟），请点击「重新发送确认码」"};if(Number(t.attempts||0)>=he)return await r.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"};if(await O(p)!==t.codeHash){t.attempts=Number(t.attempts||0)+1;let s=he-t.attempts;return s<=0?(await r.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"}):(await r.TODO_KV.put(d,JSON.stringify(t),{expirationTtl:U}),{ok:!1,error:`确认码不正确，请检查后重试（还可尝试 ${s} 次）`})}return{ok:!0}}async function ue(r,e){let a=L();return await r.TODO_KV.put(`session:${a}`,e.username,{expirationTtl:60*60*24*7}),new Response(JSON.stringify({ok:!0,username:e.username,role:e.role}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":`token=${a}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60*60*24*7}`}})}async function ee(r){let e=await r.TODO_KV.get("user:admin");if(!e){let d={username:"admin",password:await O("admin"),role:"superadmin",createdAt:new Date().toISOString()};await r.TODO_KV.put("user:admin",JSON.stringify(d));return}let a=JSON.parse(e);a.role==="admin"&&(a.role="superadmin",await r.TODO_KV.put("user:admin",JSON.stringify(a)))}async function Oe(r,e){let a=await r.TODO_KV.list({prefix:"user:"}),d=[];for(let p of a.keys){let m=await r.TODO_KV.get(p.name);if(m){let t=JSON.parse(m);if(t.role==="producer"||t.role==="customer"||I(t.role)||v(t.role)||x(t)!==e)continue;let s={username:t.username,role:t.role,createdAt:t.createdAt,remark:t.remark||"",dept:t.dept||"",canPlaceOrder:oe(t)};t.role==="restricted"&&(s.watched=await W(r,t.username)),s.mentionsUnread=await it(r,t.username),d.push(s)}}return d}async function N(r,e,a){let d=await r.TODO_KV.get(`user:${e}`);if(!d)return null;let p=JSON.parse(d);return I(p.role)||x(p)!==a?null:p}async function rt(r){let e=await r.TODO_KV.list({prefix:"user:"}),a=[];for(let d of e.keys){let p=await r.TODO_KV.get(d.name);if(!p)continue;let m=JSON.parse(p);v(m.role)&&a.push({username:m.username,teamName:m.teamName||m.username,contact:m.contact||"",email:m.email||"",remark:m.remark||"",plan:Y(m)?"pro":"trial",pro:E(m),status:G(m),rawStatus:m.status||"active",emailVerified:m.emailVerified!==!1,emailVerifiedAt:m.emailVerifiedAt||"",trialEndsAt:m.trialEndsAt||"",expiresAt:Y(m)&&m.expiresAt||"",createdAt:m.createdAt,renewedAt:m.renewedAt||"",subscribeRequest:Ce(m)})}return a.sort((d,p)=>{let m=d.subscribeRequest&&d.subscribeRequest.at?1:0,t=p.subscribeRequest&&p.subscribeRequest.at?1:0;return m!==t?t-m:d.createdAt<p.createdAt?1:-1}),a}async function V(r,e){let a=await r.TODO_KV.get(`todos:${e}`);return a?JSON.parse(a):[]}async function H(r,e,a){await r.TODO_KV.put(`todos:${e}`,JSON.stringify(a))}async function j(r,e){let a=await r.TODO_KV.get(`customers:${e}`);return a?JSON.parse(a):[]}async function ge(r,e,a){await r.TODO_KV.put(`customers:${e}`,JSON.stringify(a))}async function R(r,e){let a=!e||e===_?"customerList":`customerList:${e}`,d=await r.TODO_KV.get(a);return d?JSON.parse(d):[]}async function se(r,e,a){let d=!e||e===_?"customerList":`customerList:${e}`;await r.TODO_KV.put(d,JSON.stringify(a))}async function nt(r,e,a){let d=String(a||"").trim();if(!d)return;let p=await j(r,e.username);p.some(s=>s.name===d)||(p.push({id:L().slice(0,12),name:d,createdAt:new Date().toISOString()}),await ge(r,e.username,p));let m=x(e),t=await R(r,m);t.some(s=>s.name===d)||(t.push({id:L().slice(0,12),name:d,description:"",createdAt:new Date().toISOString()}),await se(r,m,t))}var Ne=["self","purchased"],at="self";function Be(r){let e=String(r??"").trim();return Ne.includes(e)?e:at}async function M(r,e){let a=!e||e===_?"producers":`producers:${e}`,d=await r.TODO_KV.get(a);return(d?JSON.parse(d):[]).map(m=>({id:m.id,username:m.username||m.shortName||"",description:m.description||"",nature:Be(m.nature),createdAt:m.createdAt}))}async function te(r,e,a){let d=!e||e===_?"producers":`producers:${e}`;await r.TODO_KV.put(d,JSON.stringify(a))}async function W(r,e){let a=await r.TODO_KV.get(`watch:${e}`);return a?JSON.parse(a):[]}async function Te(r,e,a){await r.TODO_KV.put(`watch:${e}`,JSON.stringify(a))}var st=50;function ot(r){let e=new Set,a=/@([A-Za-z0-9_.-]{3,20})/g,d=String(r||""),p;for(;(p=a.exec(d))!==null;)e.add(p[1]);return[...e]}async function Se(r,e){let a=[],d=await A(r,e);d&&a.push({username:d.username,role:d.role,isTeamAdmin:!0,label:d.teamName||d.username});let p=await Oe(r,e);for(let m of p)a.push({username:m.username,role:m.role,dept:m.dept||"",isTeamAdmin:!1,label:m.username});return a}async function ie(r,e){let a=await r.TODO_KV.get(`mentions:${e}`);return a?JSON.parse(a):[]}async function Me(r,e,a){await r.TODO_KV.put(`mentions:${e}`,JSON.stringify(a.slice(0,st)))}async function Ae(r,e){let a=await ie(r,e);return{unread:a.filter(d=>!d.read).length,total:a.length}}async function it(r,e){return(await Ae(r,e)).unread}async function dt(r,e,a,d){if(!a||!a.length)return;let p=new Date().toISOString();for(let m of a){let t=await ie(r,m);t.unshift({id:L().slice(0,12),from:e.username,todoId:d.todoId,todoTitle:d.todoTitle||"",todoOwner:d.owner||"",text:d.text||"",at:p,read:!1}),await Me(r,m,t)}}var ct=["生产部","计划部","采购部","品质部","财务部"],lt="生产部";function ut(r,e){if(e!=="restricted")return"";let a=String(r??"").trim();return ct.includes(a)?a:lt}var mt={superadmin:"超级管理员",team:"团队管理员",editor:"业务部",viewer:"业务主管",restricted:"生产部",producer:"生产方",customer:"客户",superviewer:"总经理"};function re(r){return r&&(r.dept||mt[r.role]||r.role)||""}function me(r){return r==="viewer"||r==="restricted"||r==="producer"||r==="customer"}function oe(r){return r?v(r.role)?!0:typeof r.canPlaceOrder=="boolean"?r.canPlaceOrder:r.role==="editor"||r.role==="member":!1}function F(r){return v(r)||r==="superviewer"}async function Q(r,e,a=!1,d=null){let p=await r.TODO_KV.list({prefix:"todos:"}),m=p.keys.map(o=>o.name.replace("todos:","")),t=await Promise.all(m.map(async o=>{let i=await r.TODO_KV.get(`user:${o}`);return i?JSON.parse(i):null})),s=[];for(let o=0;o<m.length;o++){let i=t[o];if(!i||x(i)!==e)continue;let l=await r.TODO_KV.get(p.keys[o].name),c=l?JSON.parse(l):[];for(let u of c){let f=u.status||(u.done?"done":"pending");a&&f==="pending"||d&&!d.includes(u.producerId)||s.push({...u,status:f,owner:m[o]})}}return s.sort((o,i)=>o.createdAt<i.createdAt?1:-1),s}async function pt(r,e){let a=await r.TODO_KV.list({prefix:"user:"}),d=[];for(let p of a.keys){let m=await r.TODO_KV.get(p.name);if(!m)continue;let t=null;try{t=JSON.parse(m)}catch{continue}!t||I(t.role)||x(t)===e&&d.push({username:t.username||p.name.replace(/^user:/,""),role:t.role})}return d}async function ft(r,e){let a=0,d=await r.TODO_KV.get(`todos:${e}`);if(d)try{let p=JSON.parse(d);a=Array.isArray(p)?p.length:0}catch{a=0}return await r.TODO_KV.delete(`user:${e}`),await r.TODO_KV.delete(`todos:${e}`),await r.TODO_KV.delete(`customers:${e}`),await r.TODO_KV.delete(`watch:${e}`),await r.TODO_KV.delete(`mentions:${e}`),await r.TODO_KV.delete(`emailcode:${e}`),await r.TODO_KV.delete(`mailrate:${e}`),{todos:a}}async function gt(r,e,a){let d=r.method;if(a==="/api/login"&&d==="POST"){await ee(e);let{username:t,password:s}=await k(r);if(!t||!s)return n({error:"请输入用户名和密码"},400);let o=await e.TODO_KV.get(`user:${t}`);if(!o)return n({error:"用户名或密码错误"},401);let i=JSON.parse(o);if(await O(s)!==i.password)return n({error:"用户名或密码错误"},401);if(!I(i.role)){let c=await A(e,x(i));if(c&&G(c)==="disabled")return n({error:"该账户已被停用，请联系超级管理员开通"},403)}if(i.role==="team"&&i.emailVerified===!1){let c=await et(e,i,r);return n({ok:!1,needVerify:!0,username:i.username,email:J(i.email||""),codeTtlSec:U,resendAfterSec:ne,devCode:c.devCode||void 0,sendError:c.ok?void 0:c.error,message:c.ok?`确认码${c.resent?"已重新发送":"已发送"}至注册邮箱，请在 ${Math.floor(U/60)} 分钟内输入完成确认`:"确认码发送失败，请稍后点「重新发送确认码」重试"})}return ue(e,i)}if(a==="/api/register"&&d==="POST"){if(await ee(e),!(await q(e)).allowRegister)return n({error:"系统当前已关闭新用户注册，如需开通请联系超级管理员"},403);let{teamName:s,username:o,email:i,password:l,confirmPassword:c,contact:u,remark:f}=await k(r),g=(o||"").trim(),y=(s||"").trim(),w=String(i||"").trim(),C=String(l||"");if(!y)return n({error:"请输入团队名称"},400);if(!g)return n({error:"请输入登录用户名"},400);if(!/^[A-Za-z0-9_.-]{3,20}$/.test(g))return n({error:"用户名为 3~20 位字母、数字、下划线、点或短横线"},400);if(g==="admin")return n({error:"该用户名不可用，请更换"},400);if(!w)return n({error:"请输入邮箱"},400);if(w.length>60)return n({error:"邮箱长度不能超过 60 个字符"},400);if(!K.test(w))return n({error:"邮箱格式不正确，请检查后重试"},400);if(!C)return n({error:"请输入密码"},400);if(C.length<6)return n({error:"密码至少 6 位"},400);if(c!==void 0&&C!==String(c))return n({error:"两次输入的密码不一致"},400);if(await e.TODO_KV.get(`user:${g}`))return n({error:"该用户名已被占用，请更换"},400);let T=new Date,S={username:g,password:await O(C),role:"team",teamId:g,teamName:y,contact:String(u||"").trim(),email:w,remark:String(f||"").trim(),emailVerified:!1,plan:"trial",status:"active",trialEndsAt:"",expiresAt:"",createdAt:T.toISOString()};await e.TODO_KV.put(`user:${g}`,JSON.stringify(S));let B=await fe(e,S,r);return B.ok?n({ok:!0,needVerify:!0,username:g,role:"team",teamName:y,email:J(w),codeTtlSec:U,resendAfterSec:ne,devCode:B.devCode||void 0,unlimitedTrial:!0,message:`确认码已发送至 ${J(w)}，请在 ${Math.floor(U/60)} 分钟内输入完成邮箱确认`}):(await e.TODO_KV.delete(`user:${g}`),n({error:"注册未完成："+B.error},503))}if(a==="/api/register/verify"&&d==="POST"){await ee(e);let{username:t,code:s}=await k(r),o=String(t||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return ue(e,l);let c=await tt(e,l,s);return c.ok?(l.emailVerified=!0,l.emailVerifiedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${o}`,JSON.stringify(l)),await e.TODO_KV.delete(`emailcode:${o}`),ue(e,l)):n({error:c.error},400)}if(a==="/api/register/resend"&&d==="POST"){await ee(e);let{username:t,password:s}=await k(r),o=String(t||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return n({error:"该账号已完成邮箱确认，可直接登录"},400);if(!s||await O(String(s))!==l.password)return n({error:"密码不正确，无法重新发送确认码"},401);let c=await fe(e,l,r);return c.ok?n({ok:!0,email:J(l.email||""),codeTtlSec:U,resendAfterSec:ne,devCode:c.devCode||void 0,message:`确认码已重新发送至 ${J(l.email||"")}`}):n({error:c.error},429)}if(a==="/api/logout"&&d==="POST"){let t=Ie(r);return t&&await e.TODO_KV.delete(`session:${t}`),new Response(JSON.stringify({ok:!0}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":"token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"}})}if(a==="/api/me"&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s={username:t.username,role:t.role};if(s.canPlaceOrder=oe(t),v(t.role)){s.teamId=t.teamId||t.username,s.teamName=t.teamName||t.username,s.contact=t.contact||"",s.plan=Y(t)?"pro":"trial",s.pro=E(t),s.trialEndsAt=t.trialEndsAt||"",s.expiresAt=t.expiresAt||"",s.status=G(t);let i=Ce(t);s.subscribeRequest=i,s.renewRequest=i}else{let i=await A(e,x(t));s.teamId=x(t),s.teamName=i?i.teamName||i.username:"",s.teamPro=E(i)}let o=await Ae(e,t.username);return s.mentionsUnread=o.unread,s.mentionsTotal=o.total,n(s)}if(a==="/api/change-password"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let{oldPassword:s,newPassword:o}=await k(r);return!s||!o?n({error:"请填写完整"},400):await O(s)!==t.password?n({error:"原密码错误"},400):(t.password=await O(o),await e.TODO_KV.put(`user:${t.username}`,JSON.stringify(t)),n({ok:!0}))}if(a==="/api/settings"&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=await q(e),o=v(t.role)?t:await A(e,x(t));return n({settings:{siteName:s.siteName,allowRegister:s.allowRegister,supportEmail:s.supportEmail,favicon:s.favicon,teamName:o?o.teamName||o.username:""}})}if(a==="/api/settings"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let{siteName:s,teamName:o,allowRegister:i,supportEmail:l,favicon:c}=await k(r);if(o!==void 0){if(!v(t.role))return n({error:"无权限"},403);let y=String(o).trim();return y?(t.teamName=y,await e.TODO_KV.put(`user:${t.username}`,JSON.stringify(t)),n({ok:!0,settings:{teamName:y}})):n({error:"请输入团队名称"},400)}if(s===void 0&&i===void 0&&l===void 0&&c===void 0)return n({error:"请填写要保存的内容"},400);if(!I(t.role))return n({error:"无权限"},403);let u=await e.TODO_KV.get("settings"),f=u?JSON.parse(u):{};if(s!==void 0){if(!s||!s.trim())return n({error:"请输入网站名称"},400);f.siteName=s.trim()}if(i!==void 0&&(f.allowRegister=i===!0||i==="true"),l!==void 0){let y=String(l||"").trim();if(y.length>60)return n({error:"联系邮箱长度不能超过 60 个字符"},400);if(y&&!K.test(y))return n({error:"联系邮箱格式不正确，请检查后重试（留空则不显示该提示）"},400);f.supportEmail=y}if(c!==void 0){let y=String(c||"").trim();if(y.length>300)return n({error:"网站图标链接不能超过 300 个字符"},400);let w=z(y);if(w===null)return n({error:"网站图标需填写以 http:// 或 https:// 开头的图片链接（留空则使用默认图标）"},400);f.favicon=w}await e.TODO_KV.put("settings",JSON.stringify(f));let g=await q(e);return n({ok:!0,settings:{siteName:g.siteName,allowRegister:g.allowRegister,supportEmail:g.supportEmail,favicon:g.favicon}})}if((a==="/api/subscribe-request"||a==="/api/renew-request")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);let{note:s,contact:o,plan:i}=await k(r);o!==void 0&&(t.contact=String(o||"").trim().slice(0,50));let l=i&&typeof i=="object"?i:null,c=l&&Number(l.days)>0?Math.floor(Number(l.days)):0,u={at:new Date().toISOString(),kind:E(t)?"renew":"subscribe",plan:l&&c?{id:String(l.id||"").slice(0,20),term:String(l.term||"").slice(0,20),price:Number(l.price)>=0?Number(l.price):0,days:c}:null,note:String(s||"").trim().slice(0,200),contact:t.contact||""};return t.subscribeRequest=u,delete t.renewRequest,await e.TODO_KV.put(`user:${t.username}`,JSON.stringify(t)),n({ok:!0,subscribeRequest:u})}let p="/api/teams";if(a===p&&d==="GET"){let t=await h(r,e);return t?I(t.role)?n({teams:await rt(e)}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a.startsWith(`${p}/`)&&a.endsWith("/status")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${p}/`,"").replace("/status","")),{status:o}=await k(r);if(!["active","disabled"].includes(o))return n({error:"无效的状态"},400);let i=await A(e,s);return i?(i.status=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,status:G(i),expiresAt:i.expiresAt||i.trialEndsAt||""})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${p}/`)&&a.endsWith("/renew")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${p}/`,"").replace("/renew","")),o=await k(r),i=Number(o.days)>0?Math.floor(Number(o.days)):30;if(i>3650)return n({error:"续费天数过大"},400);let l=await A(e,s);if(!l)return n({error:"团队用户不存在"},404);let c=Date.now(),u=new Date(l.expiresAt||"").getTime(),f=Number.isNaN(u)||u<c?c:u,g=new Date(f+i*Re).toISOString();return l.expiresAt=g,l.status="active",l.plan="pro",l.renewedAt=new Date().toISOString(),delete l.subscribeRequest,delete l.renewRequest,await e.TODO_KV.put(`user:${s}`,JSON.stringify(l)),n({ok:!0,days:i,expiresAt:g,status:"active",pro:!0})}if(a.startsWith(`${p}/`)&&a.endsWith("/reset-password")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${p}/`,"").replace("/reset-password","")),{newPassword:o}=await k(r);if(!o||!String(o).trim())return n({error:"请输入新密码"},400);let i=await A(e,s);return i?(i.password=await O(String(o).trim()),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${p}/`)&&a.endsWith("/remark")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${p}/`,"").replace("/remark","")),{remark:o}=await k(r),i=await A(e,s);return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${p}/`)&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.slice(p.length+1));if(!s||s.indexOf("/")!==-1)return n({error:"接口不存在"},404);let o=await A(e,s);if(!o)return n({error:"团队用户不存在"},404);if(o.status!=="disabled")return n({error:"请先「停用」该团队用户，确认无误后再删除"},400);let i=await pt(e,s),l=i.reduce((g,y)=>(g[y.role]=(g[y.role]||0)+1,g),{}),c=0;for(let g of i){let y=await ft(e,g.username);c+=y.todos}let u=await M(e,s),f=await R(e,s);return await e.TODO_KV.delete(`producers:${s}`),await e.TODO_KV.delete(`customerList:${s}`),n({ok:!0,deleted:{team:s,teamName:o.teamName||s,accounts:i.length,members:i.filter(g=>!["team","producer","customer"].includes(g.role)).length,producers:u.length,customers:f.length,todos:c,roles:l}})}if(a==="/api/admin/email-settings"&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let s=await ae(e);return n({settings:Ee(s,e)})}if(a==="/api/admin/email-settings"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let{from:s,fromName:o,apiKey:i,devMode:l,smtpHost:c,smtpPort:u,smtpUser:f,smtpPass:g}=await k(r),y=await e.TODO_KV.get("emailSettings"),w=y?JSON.parse(y):{};if(s!==void 0){let b=String(s||"").trim();if(b&&!K.test(b))return n({error:"发件邮箱格式不正确，请检查后重试"},400);w.from=b}if(o!==void 0&&(w.fromName=String(o||"").trim().slice(0,30)),i!==void 0){let b=String(i||"").trim();b==="-"?delete w.apiKey:b&&(w.apiKey=b)}if(c!==void 0){let b=String(c||"").trim();if(b&&!/^[A-Za-z0-9.-]{1,60}$/.test(b))return n({error:"SMTP 服务器地址格式不正确（例如 smtp.qq.com）"},400);w.smtpHost=b||"smtp.qq.com"}if(u!==void 0){let b=String(u??"").trim(),T=Number(b)||0;if(b&&(T<1||T>65535))return n({error:"SMTP 端口需为 1~65535 的数字（QQ 邮箱用 465，或 587 + STARTTLS）"},400);w.smtpPort=T||465}if(f!==void 0){let b=String(f||"").trim();if(b&&!K.test(b))return n({error:"SMTP 账号需填写完整邮箱地址（例如 xxx@qq.com）"},400);w.smtpUser=b}if(g!==void 0){let b=String(g||"").trim();b==="-"?delete w.smtpPass:b&&(w.smtpPass=b)}l!==void 0&&(w.devMode=!!l),await e.TODO_KV.put("emailSettings",JSON.stringify(w));let C=await ae(e);return n({ok:!0,settings:Ee(C,e)})}if(a==="/api/admin/email-test"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!I(t.role))return n({error:"无权限"},403);let{to:s}=await k(r),o=String(s||"").trim();if(!o)return n({error:"请输入测试收件邮箱"},400);if(!K.test(o))return n({error:"测试收件邮箱格式不正确"},400);let i=await ae(e);if(i.devMode)return n({ok:!0,devMode:!0,message:"当前为调试模式（不真实发送邮件），无需测试发信"});if(!i.provider)return n({error:"邮件服务未配置：请填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）并保存后，再发送测试邮件"},400);let l=i.provider==="smtp"?"QQ 邮箱 SMTP":i.provider==="resend"?"Resend":"Cloudflare 邮件绑定",c=await Pe(e,i,{to:o,subject:"【测试】邮件服务配置正常",text:"这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。",html:"<p>这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。</p>"},{local:pe(r)});return c.ok?n({ok:!0,message:`测试邮件已通过 ${l} 发送至 ${o}，请查收（若没收到，请检查垃圾邮件箱）`}):n({error:c.error},400)}if(a==="/api/users"&&d==="GET"){let t=await h(r,e);return t?v(t.role)?E(t)?n({users:await Oe(e,x(t))}):P():n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/users"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let{username:s,password:o,role:i,dept:l}=await k(r);if(!s||!o)return n({error:"请填写用户名和密码"},400);let c=["restricted","superviewer"].includes(i)?i:"editor";if(await e.TODO_KV.get(`user:${s}`))return n({error:"用户名已存在"},400);let f={username:s,password:await O(o),role:c,teamId:x(t),createdAt:new Date().toISOString()},g=ut(l,c);return g&&(f.dept=g),await e.TODO_KV.put(`user:${s}`,JSON.stringify(f)),n({ok:!0})}if(a.startsWith("/api/users/")&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace("/api/users/",""));return await N(e,s,x(t))?(await e.TODO_KV.delete(`user:${s}`),await e.TODO_KV.delete(`todos:${s}`),await e.TODO_KV.delete(`customers:${s}`),await e.TODO_KV.delete(`watch:${s}`),await e.TODO_KV.delete(`mentions:${s}`),n({ok:!0})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/reset-password")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/users/","").replace("/reset-password","")),{newPassword:i}=await k(r);if(!i||!i.trim())return n({error:"请输入新密码"},400);let l=await e.TODO_KV.get(`user:${o}`);if(!l){let f=(await M(e,s)).find(w=>w.username===o);if(f)return await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await O(i),role:"producer",producerId:f.id,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0});let y=(await R(e,s)).find(w=>w.name===o);return y?(await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await O(i),role:"customer",customerId:y.id,customerName:y.name,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0})):n({error:"成员不存在"},404)}let c=JSON.parse(l);return I(c.role)||x(c)!==s?n({error:"成员不存在"},404):(c.password=await O(i),await e.TODO_KV.put(`user:${o}`,JSON.stringify(c)),n({ok:!0}))}if(a.startsWith("/api/users/")&&a.endsWith("/remark")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace("/api/users/","").replace("/remark","")),{remark:o}=await k(r),i=await N(e,s,x(t));return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/order-permission")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace("/api/users/","").replace("/order-permission","")),{canPlaceOrder:o}=await k(r);if(typeof o!="boolean")return n({error:"请传入 true（有）或 false（无）"},400);let i=await N(e,s,x(t));return i?(i.canPlaceOrder=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,canPlaceOrder:o})):n({error:"成员不存在"},404)}if(a.startsWith("/api/customers/")&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/customers/",""));return s!==t.username&&!(v(t.role)?await N(e,s,x(t)):null)?n({error:"无权限"},403):n({customers:await j(e,s)})}if(a.startsWith("/api/customers/")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace("/api/customers/",""));if(!await N(e,s,x(t)))return n({error:"成员不存在"},404);let{name:i,globalId:l}=await k(r);if(!i||!i.trim())return n({error:"请输入客户名称"},400);let c=await j(e,s),u=i.trim(),f=l?String(l).trim():"";if(c.some(y=>y.name===u||f&&y.id===f))return n({error:"客户已存在"},400);let g={id:f||L().slice(0,12),name:u,createdAt:new Date().toISOString()};return c.push(g),await ge(e,s,c),n({ok:!0,customer:g})}if(a.startsWith("/api/customers/")&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace("/api/customers/","")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await N(e,i,x(t)))return n({error:"成员不存在"},404);let u=await j(e,i);return u=u.filter(f=>f.id!==l),await ge(e,i,u),n({ok:!0})}if(a==="/api/customer-list"&&d==="GET"){let t=await h(r,e);return t?!v(t.role)||!E(t)?P():n({customers:await R(e,x(t))}):n({error:"未登录"},401)}if(a==="/api/customer-list"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);let s=x(t),{name:o,password:i,description:l}=await k(r);if(!o||!o.trim())return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let c=await R(e,s),u=o.trim();if(c.some(y=>y.name===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被占用"},400);let g={id:L().slice(0,12),name:u,description:(l||"").trim(),createdAt:new Date().toISOString()};return c.push(g),await se(e,s,c),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await O(String(i).trim()),role:"customer",customerId:g.id,customerName:u,teamId:s,createdAt:g.createdAt})),n({ok:!0,customer:g})}if(a.startsWith("/api/customer-list/")&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/customer-list/","")),i=await R(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await se(e,s,i),l&&l.name&&await e.TODO_KV.delete(`user:${l.name}`),n({ok:!0})}if(a.startsWith("/api/customer-list/")&&a.endsWith("/description")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/customer-list/","").replace("/description","")),{description:i}=await k(r),l=await R(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await se(e,s,l),n({ok:!0,description:c.description})):n({error:"客户不存在"},404)}if(a==="/api/producers"&&d==="GET"){let t=await h(r,e);return t?n({producers:await M(e,x(t))}):n({error:"未登录"},401)}if(a==="/api/producers"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),{username:o,password:i,description:l,nature:c}=await k(r),u=(o||"").trim();if(!u)return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let f=await M(e,s);if(f.some(w=>w.username===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被成员占用"},400);let y={id:L().slice(0,12),username:u,description:(l||"").trim(),nature:Be(c),createdAt:new Date().toISOString()};return f.push(y),await te(e,s,f),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await O(String(i).trim()),role:"producer",producerId:y.id,teamId:s,createdAt:y.createdAt})),n({ok:!0,producer:y})}if(a.startsWith("/api/producers/")&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/producers/","")),i=await M(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await te(e,s,i),l&&l.username&&await e.TODO_KV.delete(`user:${l.username}`),n({ok:!0})}if(a.startsWith("/api/producers/")&&a.endsWith("/description")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/producers/","").replace("/description","")),{description:i}=await k(r),l=await M(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await te(e,s,l),n({ok:!0,description:c.description})):n({error:"生产方不存在"},404)}if(a.startsWith("/api/producers/")&&a.endsWith("/nature")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace("/api/producers/","").replace("/nature","")),{nature:i}=await k(r),l=String(i??"").trim();if(!Ne.includes(l))return n({error:"生产方性质只能是「自产」或「外购」"},400);let c=await M(e,s),u=c.find(f=>f.id===o);return u?(u.nature=l,await te(e,s,c),n({ok:!0,nature:u.nature})):n({error:"生产方不存在"},404)}let m="/api/watch/";if(a.startsWith(m)&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=x(t),o=decodeURIComponent(a.replace(m,""));if(o!==t.username&&!(v(t.role)?await N(e,o,s):null))return n({error:"无权限"},403);let i=await W(e,o),l=await M(e,s),c=i.map(u=>{let f=l.find(g=>g.id===u);return f||{id:u,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:c})}if(a.startsWith(m)&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=x(t),o=decodeURIComponent(a.replace(m,"")),i=await N(e,o,s);if(!i)return n({error:"成员不存在"},404);let{producerId:l}=await k(r);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await M(e,s)).some(g=>g.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);if(i.role!=="restricted")return n({error:"该成员不是「生产部」成员，无法授权可观察的生产方"},400);let f=await W(e,o);return f.includes(c)?n({error:"已授权该生产方"},400):(f.push(c),await Te(e,o,f),n({ok:!0,producerIds:f}))}if(a.startsWith(m)&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!v(t.role))return n({error:"无权限"},403);if(!E(t))return P();let s=decodeURIComponent(a.replace(m,"")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await N(e,i,x(t)))return n({error:"成员不存在"},404);let u=await W(e,i);return u=u.filter(f=>f!==l),await Te(e,i,u),n({ok:!0,producerIds:u})}if(a==="/api/team-members"&&d==="GET"){let t=await h(r,e);return t?n({members:await Se(e,x(t))}):n({error:"未登录"},401)}if(a==="/api/mentions"&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=await ie(e,t.username);return n({items:s,unread:s.filter(o=>!o.read).length,total:s.length})}if(a==="/api/mentions/read"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=await k(r),o=s&&s.id?String(s.id).trim():"",i=!(s&&s.read===!1),l=await ie(e,t.username),c=0;for(let u of l)u.read!==i&&(o&&u.id!==o||(u.read=i,c++));return c&&await Me(e,t.username,l),n({ok:!0,changed:c,read:i,unread:l.filter(u=>!u.read).length,total:l.length})}if(a==="/api/todos"&&d==="GET"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=x(t);if(F(t.role))return n({todos:await Q(e,s,!1),readonly:!1,allUsers:!0});if(t.role==="viewer")return n({todos:await Q(e,s,!0),readonly:!0,allUsers:!0});if(t.role==="restricted"){let o=await W(e,t.username);return n({todos:await Q(e,s,!0,o),readonly:!0,allUsers:!0})}if(t.role==="producer"){let i=(await M(e,s)).find(l=>l.id===t.producerId);return n({todos:await Q(e,s,!0,i?[i.id]:[]),readonly:!0,allUsers:!0})}if(t.role==="customer"){let o=t.customerName||"",i=o?await Q(e,s,!0):[];return n({todos:i.filter(l=>l.customer===o),readonly:!0,allUsers:!0})}return n({todos:await V(e,t.username),readonly:!1})}if(a==="/api/todos"&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!oe(t))return n({error:re(t)+"无「生产单下单权限」，请联系团队管理员在「成员管理」中开通"},403);let{title:s,customer:o,dueDate:i,amount:l,orderUrl:c,purchaseUrl:u,currency:f}=await k(r);if(!o||!o.trim())return n({error:"选择客户"},400);if(!s||!s.trim())return n({error:"请输入主题"},400);if(!i||!String(i).trim())return n({error:"选择交期"},400);let g=z(c);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);let y=z(u);if(y===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);if(l==null||String(l).trim()==="")return n({error:"输入金额"},400);let w=Number(l);if(Number.isNaN(w)||w<0)return n({error:"金额必须为非负数字"},400);let C=o.trim();if(v(t.role)&&!E(t))await nt(e,t,C);else if(v(t.role)){let S=await R(e,x(t)),B=await j(e,t.username);if(!S.some(D=>D.name===C)&&!B.some(D=>D.name===C))return n({error:"客户不存在，请先在「客户管理」中添加"},400)}else if(!(await j(e,t.username)).some(B=>B.name===C))return n({error:"客户不存在，请联系管理员添加"},400);let b=await V(e,t.username),T={id:L().slice(0,12),customer:o.trim(),title:s.trim(),dueDate:String(i).trim(),amount:w,currency:qe(f),orderUrl:g,purchaseUrl:y,notes:[],status:"pending",done:!1,createdAt:new Date().toISOString()};return b.unshift(T),await H(e,t.username,b),n({ok:!0,todo:T})}if(a.startsWith("/api/todos/")&&a.endsWith("/notes")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/notes","")),o=await k(r),i=o.text;if(!i||!i.trim())return n({error:"请输入备注内容"},400);let l=t.username;if(me(t.role)||F(t.role)){let b=o.owner||t.username;if(b!==t.username&&!await N(e,b,x(t)))return n({error:"无权操作该用户的待办"},403);l=b}let c=await V(e,l),u=c.findIndex(b=>b.id===s);if(u===-1)return n({error:"未找到"},404);Array.isArray(c[u].notes)||(c[u].notes=[]);let f=await Se(e,x(t)),g=new Set(f.map(b=>b.username)),y=new Set;if(Array.isArray(o.mentions))for(let b of o.mentions){let T=String(b||"").trim();T&&g.has(T)&&y.add(T)}for(let b of ot(i))g.has(b)&&y.add(b);y.delete(t.username);let w=[...y],C={id:L().slice(0,12),text:i.trim(),author:t.username,createdAt:new Date().toISOString(),mentions:w};return c[u].notes.push(C),await H(e,l,c),await dt(e,t,w,{todoId:c[u].id,todoTitle:c[u].title,owner:l,text:C.text}),n({ok:!0,note:C})}if(a.startsWith("/api/todos/")&&d==="PUT"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(me(t.role))return n({error:re(t)+"无编辑权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=await k(r);if((typeof o.status=="string"||typeof o.done=="boolean")&&!F(t.role))return n({error:"只有团队管理员或总经理可以改变待办状态"},403);let l=t.username;if(F(t.role)&&o.owner&&o.owner!==t.username){if(!await N(e,o.owner,x(t)))return n({error:"无权操作该用户的待办"},403);l=o.owner}let c=await V(e,l),u=c.findIndex(f=>f.id===s);if(u===-1)return n({error:"未找到"},404);if(typeof o.title=="string"){let f=o.title.trim();if(!f)return n({error:"请输入 PO# / 主题"},400);c[u].title=f}if(typeof o.dueDate=="string"||o.amount!==void 0||o.orderUrl!==void 0||o.purchaseUrl!==void 0){if((c[u].status||(c[u].done?"done":"pending"))!=="pending")return n({error:"仅「待确认」的待办可修改交期/金额/文件链接"},403);if(typeof o.dueDate=="string"){let g=o.dueDate.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(g))return n({error:"请输入正确的交期"},400);c[u].dueDate=g}if(o.amount!==void 0&&o.amount!==null&&String(o.amount).trim()!==""){let g=Number(o.amount);if(Number.isNaN(g)||g<0)return n({error:"金额必须为非负数字"},400);c[u].amount=g}if(o.orderUrl!==void 0){let g=z(o.orderUrl);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);c[u].orderUrl=g}if(o.purchaseUrl!==void 0){let g=z(o.purchaseUrl);if(g===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);c[u].purchaseUrl=g}}if(typeof o.producerId=="string"&&o.producerId.trim()){let g=(await M(e,x(t))).find(y=>y.id===o.producerId.trim());if(!g)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);c[u].producerId=g.id,c[u].producerName=g.username,c[u].producerAssignedAt=new Date().toISOString()}if(typeof o.status=="string"){if(!["pending","doing","done"].includes(o.status))return n({error:"无效的状态"},400);c[u].status=o.status,c[u].done=o.status==="done"}else typeof o.done=="boolean"&&(c[u].done=o.done,c[u].status=o.done?"done":"doing");return c[u].status==="doing"&&!c[u].producerId&&await Je(e,t)?n({error:"请为该待办指定生产方（生产方来自「生产方管理」）"},400):(await H(e,l,c),n({ok:!0,todo:c[u]}))}if(a.startsWith("/api/todos/")&&a.endsWith("/purchase-link")&&d==="POST"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(!oe(t))return n({error:re(t)+"无「生产单下单权限」，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/purchase-link","")),o=await k(r),i=t.username;if(o.owner&&o.owner!==t.username){if(!await N(e,o.owner,x(t)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await V(e,i),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].purchaseUrl)return n({error:"该订单已有采购文件链接，如需修改请联系团队管理员"},403);let u=z(o.purchaseUrl);return u===null?n({error:"采购文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].purchaseUrl=u,await H(e,i,l),n({ok:!0,purchaseUrl:u})):n({error:"请输入采购文件链接"},400)}if(a.startsWith("/api/todos/")&&d==="DELETE"){let t=await h(r,e);if(!t)return n({error:"未登录"},401);if(me(t.role))return n({error:re(t)+"无删除权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=t.username;if(F(t.role)){let u=new URL(r.url).searchParams.get("owner");if(u&&u!==t.username){if(!await N(e,u,x(t)))return n({error:"无权操作该用户的待办"},403);o=u}}let i=await V(e,o),l=i.find(u=>u.id===s);return l?(l.status||(l.done?"done":"pending"))!=="pending"?n({error:"该事件已进入「进行中/已完成」状态，无法删除"},403):(i=i.filter(u=>u.id!==s),await H(e,o,i),n({ok:!0})):n({error:"未找到"},404)}return n({error:"接口不存在"},404)}var ht={async fetch(r,e,a){let d=new URL(r.url),p=d.pathname;if(p.startsWith("/api/"))try{return await gt(r,e,p)}catch(m){return n({error:"服务器错误: "+m.message},500)}if(p==="/"||p==="/login"){let m=await q(e);return new Response(be(m.siteName,m.allowRegister,m.supportEmail,m.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(p==="/todos"){let m=await q(e);return new Response(ye(m.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(p==="/admin"){let m=await h(r,e);if(!m)return Response.redirect(d.origin+"/",302);if(!I(m.role))return Response.redirect(d.origin+"/todos",302);let t=await q(e);return new Response(we(t.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}return new Response("Not Found",{status:404})}};export{ht as default};
